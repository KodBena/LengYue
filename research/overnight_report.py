...
def main() -> None:
    ...
    L.append("---")
    L.append("")
    L.append("## Morning verdict (overnight findings, before deeper inspection)")
    L.append("")
    ...
    L.append("")
    L.append("- **Hyperparameter sweep: +0.04 OOD R² achievable.** The default LightGBMWrap is slightly over-regularized; `num_leaves=8, min_data=3, lr=0.1, λ=0` gives OOD R²=+0.547 vs default +0.510 on the anchor cell. Worth applying to the production allocator predictor.")
    L.append("")
    L.append("- **Recommended morning move:** the capability dispatch to the proxy (per firewall consult turn 2, §5 Tier 2) is unblocked. The architecture has empirical support; the dispatch shape (`staged_analysis` capability advertising trajectory packets at SPA-declared visit budgets) doesn't depend on which predictor/reframe variant wins long-term — only on the proxy supporting the partial-search-observation primitive. Drafting can proceed in parallel with deeper allocator refinement.")
    L.append("")
    L.append("---")
    L.append("")
