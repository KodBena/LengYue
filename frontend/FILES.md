...
│   ├── lyt-layout-types.ts            [B1]  HAND-WRITTEN (W3, ADR-0012 one-home-per-fact): the `LytProgram` type vocabulary extracted out of `lyt-layout.gen.ts`/`lyt-layout-portrait.gen.ts`, where it used to be duplicated byte-for-byte inline in both. Both generated files now `export type * from './lyt-layout-types'` to re-publish it under their existing import paths, so no consumer (LytNode.vue, useLytTrackCss.ts, useLytPresenceMenu.ts, lyt-widget-registry.ts, store/defaults.ts, App.vue) needed an import-path edit.
│   ├── lyt-widget-registry.ts         [B1]  W1 skeleton rework (ledger row 1781): hand-authored leaf-widget-id -> mount-disposition table (`mounted`/`absorbed`/`absent`) LytNode.vue and App.vue's slot wiring both consult. The DATA half of the "why slots, not a live component table" contract; see the file's own header. W2 (ledger row 1743): boardRail now actually mounts SidebarWidget (was inert, presenceDefaultVisible false with no menu to flip it on); previewBoard flips 'absent' -> 'mounted' (PreviewBoardPanel.vue).
