...
│   ├── lyt-solved-layout-asis.gen.ts  [B1]  GENERATED (research/lyt/emit_ts.py --registration current_row_asis.lyt) — solved rectangles for the LYT as-is conformance baseline (research/lyt/encodings/current_row_asis.lyt) at each representative screen size. No longer shadow-only: `layout-model.ts` imports `RESIZER_WIDTH_PX` from it (lyt-constants-swap, ledger row 1687). Do not hand-edit.
...
│   ├── lyt-solved-layout.gen.ts       [B1]  GENERATED (research/lyt/emit_ts.py, default registration) — solved rectangles for the LYT-repaired encoding (research/lyt/encodings/current_row_repaired.lyt) at each representative screen size. Phase-1 shadow mode: no runtime import from app code (frontend/scripts/lyt-conformance.mjs is the only consumer). Do not hand-edit.
