...
│   ├── ProxyUpstreamSettingField.vue  [B1]  Desktop-only (ledger rows 860-862) shared field — label/input/error/restart-notice — for the bundled proxy sidecar's upstream WebSocket URL, over useProxyUpstreamSetting.ts. ONE component mounted by BOTH wizard/steps/WizardStepEngineUri.vue and SettingsTab.vue's Session sub-tab (`field-id` prop keeps the two DOM ids distinct); owns its own IS_TAURI gate internally (v-if at its template root) so no caller can forget it. Extracted from two hand-duplicated copies after fresh-context review found the wizard's copy used a non-reactive `let` for its error state (blocker 2).
...
│   ├── useProxyUpstreamSetting.ts     [B1]  Desktop-only (ledger rows 860-862) in-app editor for the bundled proxy sidecar's upstream WebSocket URL — the cell components/ProxyUpstreamSettingField.vue (mounted by both WizardStepEngineUri.vue and SettingsTab.vue) reads/writes, via Tauri commands `get_proxy_upstream_setting`/`set_proxy_upstream_setting` (src-tauri/src/proxy_settings.rs). Validated by lib/ws-url.ts (shared with useEngineUriEditor.ts). Inert (no-op, never invokes) outside Tauri — gated on config/env.ts's `IS_TAURI`. Takes effect on next app launch, not live.
