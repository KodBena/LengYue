...
│   │   ├── ToolbarAppCluster.vue      [B3]  LYT toolbar ontology reencode (2026-08-11, ledger rows 1930/1931, item 3 "ONE APP CLUSTER"): Load/Save SGF, sliders/setup/PBO popover triggers, the engine URI editor, and the locale picker — mounted at the `.lyt` encoding's own `A_app` leaf (both screen classes). Self-contained: sources its own SGF composables rather than App.vue threading them through. Replaces the former Toolbar.vue's app-scoped half.
│   │   ├── ToolbarEngineCluster.vue   [B3]  LYT toolbar ontology reencode (2026-08-11, ledger rows 1930/1931, item 2 "ONE ENGINE CLUSTER, ENVELOPE-RESERVED"): connect/disconnect, the engine-controls button cluster (mint-card/learn-path/play/match), and engine metrics — ONE component, mounted at the `.lyt` encoding's own `A_engine` leaf (`envelope: {disconnected, connected}` — reserved at the MAX across states, so connecting never re-partitions a sibling). Reads only `isConnected`; telemetry lives in ToolbarEngineMetrics. Replaces the former Toolbar.vue's engine-scoped half.
