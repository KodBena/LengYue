...
│   ├── useProxyUpstreamSetting.ts     [B1]  Desktop-only (ledger rows 860-862) in-app editor for the bundled proxy sidecar's upstream WebSocket URL — the SAME cell WizardStepEngineUri.vue's proxy-upstream field and SettingsTab.vue's Session sub-tab both read/write, via Tauri commands `get_proxy_upstream_setting`/`set_proxy_upstream_setting` (src-tauri/src/proxy_settings.rs). Validated by lib/ws-url.ts (shared with useEngineUriEditor.ts). Inert (no-op, never invokes) outside Tauri — gated on config/env.ts's `IS_TAURI`. Takes effect on next app launch, not live.
