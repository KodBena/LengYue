...
│   │   ├── LytNode.vue                [B1]  W1 skeleton rework (ledger row 1781): generic recursive renderer for a compiled LYT Split node (`state/lyt-layout.gen.ts`) — one live CSS Grid container per H/V node, leaves/blackboxes projected through named `#leaf-<widgetId>` slots App.vue fills. Consecutive-sibling merge (registry `absorbed` status) folds adjacent tracks into one spanning mount; aspect leaves get the `.lyt-board-cell` containment wrapper. `path` prop (repair pass fix) threads each recursion's own dotted path so `domIdsByPath` resolves per-instance instead of colliding on the root's id.
...
│   │   ├── useLytTrackCss.ts          [B1]  W1 skeleton rework (ledger row 1781): pure `LytTrackShape -> CSS Grid track value` mapping (fixed/elastic/elastic-capped/board-priority-clamp), extracted out of LytNode.vue per ADR-0007. No Vue reactivity — every function is `(shape) -> string`, unit-testable directly.
...
│   ├── lyt-widget-registry.ts         [B1]  W1 skeleton rework (ledger row 1781): hand-authored leaf-widget-id -> mount-disposition table (`mounted`/`absorbed`/`absent`) LytNode.vue and App.vue's slot wiring both consult. The DATA half of the "why slots, not a live component table" contract; see the file's own header.
