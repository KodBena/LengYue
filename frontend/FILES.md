...
│   ├── ProxyUpstreamSettingField.vue  [B1]  Desktop-only (ledger rows 860-862) shared field — label/input/error/restart-notice — for the bundled proxy sidecar's upstream WebSocket URL, over useProxyUpstreamSetting.ts. ONE component mounted by BOTH wizard/steps/WizardStepEngineUri.vue and chrome/SettingsPane.vue's Session sub-tab (`field-id` prop keeps the two DOM ids distinct); owns its own IS_TAURI gate internally (v-if at its template root) so no caller can forget it. Extracted from two hand-duplicated copies after fresh-context review found the wizard's copy used a non-reactive `let` for its error state (blocker 2).
...
│   │   ├── PreviewBoardPanel.vue      [B3]  W2 (roadmap §8 W2 item 3, ledger row 1743): the `previewBoard` LYT leaf's first content — read-only MiniBoard projection of `activeBoard`'s current position (a straight port of LibraryPreviewPane's own boardSnapshot computed). DISCLOSED SCOPE NARROWING: true variation-hover content doesn't exist as readable state yet, so this mounts the active board's position as an honest placeholder — see the file's own header.
│   │   └── StatusBar.vue              [B3]  Move number, player names, komi, turn indicator, captures, transient hint, # toggle, move-navigation cluster (ToolbarMoveNav, relocated here 2026-08-11 — LYT toolbar ontology reencode item 1, "board controls go to the board").
│   │
...
│   │   ├── IntervalSummaryPanel.vue   [B3]  Compact per-colour summary table for the current analysis interval (Basic tab, on by default); looks the value up in useTriangularHeatmap's own matrix — no parallel recompute.
...
│   │   ├── BoardRailPopoverTrigger.vue [B3]  W2 (roadmap §8 W2 item 2, ledger row 1743): board-rail STYLE B — a corner-co-located badge-like trigger opening a second SidebarWidget mount as an opaque, edge-clamped popover. Rendered by App.vue only when `session.ui.railStyle === 'popover'`; the boardRail LYT leaf's own grid track stays permanently collapsed in that style.
...
│   │   ├── EngineQueueTooltip.vue     [B1]  Toolbar badge + hover panel listing in-flight KataGo queries with ETA. M2 stage B2b (2026-08-12, ledger row 2346): mounts directly at the `.lyt` encoding's own new `A_engine_queue` leaf now (no longer a child of ToolbarEngineMetrics via the retired ToolbarEngineCluster.vue).
...
│   │   ├── LocalePicker.vue           [B1]  Top-nav locale picker (flag + native name). Click-toggled dropdown menu; row 1984 (4th popover-clip class member) routed `.locale-menu` through `useFixedAnchoredPopover` (align: 'left') to escape `.lyt-toolbar-strip`'s `overflow-y: auto` clip — the composable's `open: Ref<boolean>` contract composes with the click-toggle `open` ref unmodified, no fork needed.
│   │   ├── LytNode.vue                [B1]  W1 skeleton rework (ledger row 1781): generic recursive renderer for a compiled LYT Split node (`state/lyt-layout.gen.ts`) — one live CSS Grid container per H/V node, leaves/blackboxes projected through named `#leaf-<widgetId>` slots App.vue fills. Consecutive-sibling merge (registry `absorbed` status) folds adjacent tracks into one spanning mount; aspect leaves get the `.lyt-board-cell` containment wrapper. `path` prop (repair pass fix) threads each recursion's own dotted path so `domIdsByPath` resolves per-instance instead of colliding on the root's id. W2 (ledger row 1743) adds the `presenceOverrides` prop (runtime widget-id -> visible map, generalizing the static `presenceDefaultVisible` gate) and the `board-priority-clamp` leadingReservedPx wiring (boardRail's own live width, once toggleable). Presence arc P2b (`.claude/dispatch-reports/lyt-p2b-presence-realization.md`): an Exclusive child (today: the control panel) is now a real presence-toggle target (`widgetIdOf` resolves its own `node.widget`, not `null`); when absent-but-summoned (`exclusivePopoverOpen`/`exclusivePopoverTarget` props, App.vue-driven), its content `<Teleport>`s into a caller-supplied popover container instead of unmounting outright.
│   │   ├── LytPresenceMenu.vue        [B1]  W2 (roadmap §8 W2 item 1, ledger row 1743): the corner presence menu — opaque popover listing boardRail/previewBoard/controlPanel/A_setup as checkboxes (useLytPresenceMenu.ts's own header on the four-target scope) plus an inline rail-style selector. Last-remaining-panel guard disables (never silently reverts) the sole visible target's checkbox. Presence arc P2b: a new `classDefaults` prop (App.vue's own per-class `presenceDefaultVisible` resolution) threads into useLytPresenceMenu.ts so `controlPanel`'s checkbox reflects the ACTIVE screen class's own compiled default, not a class-unaware literal, whenever no persisted user choice exists yet.
...
│   │   ├── SettingsPane.vue           [B1]  Work item `lyt-settings-live-opening` (ledger rows 2007/2009/2001): the six settings sub-tab bodies (Session (UI) / Analysis Environment / Card Sets / Advanced Registry / Analysis / Keybindings), moved verbatim from the retired SettingsTab.vue. Mounts at the LYT encoding's own `settingsPane` leaf; drives TabWidget.vue with `part="body"` (horizontal) or `part="both"` (vertical orientation, the disclosed scope-narrowing fallback — see the file's own header). Shares active sub-tab state with the sibling SettingsSubstrip.vue via `composables/chrome/useSettingsSubTab.ts`'s module-singleton ref. (Retagged B2→B1 2026-06-12 on SettingsTab.vue, inherited here: generic settings chrome; the B2/B3 contamination — the editors, profile-owner, the in-template engine.katago.analysis_env path — is a structural necessity, annotated and owned.)
│   │   ├── SettingsSubstrip.vue       [B1]  Work item `lyt-settings-live-opening` (ledger rows 2007/2009/2001): the flow-wrap-capable settings sub-tab strip (`research/lyt/flow.py`'s offline greedy-row-packing derivation, realized live via TabWidget.vue's own `wrap` prop — CSS flex-wrap). Mounts at the LYT encoding's own `settingsSubstrip` leaf; drives TabWidget.vue with `part="header"`. Renders nothing when `settingsTabsOrientation === 'vertical'` (SettingsPane.vue renders the full strip+body instead — see both files' own headers).
...
│   │   ├── SystemLogPanel.vue         [B1]  System log panel with idle row; mounted in App.vue's `#lyt-overlay-stack` when manually toggled (SystemLogToggle.vue, D2 fix) or transiently auto-revealed (useTransientLogReveal.ts) — corrected from a stale "always-visible" description.
│   │   ├── SystemLogToggle.vue        [B1]  D2 fix (`.claude/dispatch-reports/lyt-w5-parity-build.md` Defect D2): the system log's manual open/close affordance, lost in the W1 skeleton replacement and restored here. A quiet 28px corner-chrome icon button (register matches LytPresenceMenu.vue's own trigger, deliberately NOT folded into DebugMenu.vue's dev-only pill or LytPresenceMenu.vue's grid-presence-guarded popover — see its own header for the placement rationale) toggling `session.ui.systemLogExpanded` via useSystemLogToggle.ts.
│   │   ├── TabWidget.vue              [B1]  Controlled tabbed navigation.
│   │   ├── ToolbarAppCluster.vue      [B3]  LYT toolbar ontology reencode (2026-08-11, ledger rows 1930/1931, item 3 "ONE APP CLUSTER"): Load/Save SGF, sliders/PBO popover triggers, the engine URI editor, and the locale picker — mounted at the `.lyt` encoding's own `A_app` leaf (both screen classes). Self-contained: sources its own SGF composables rather than App.vue threading them through. Replaces the former Toolbar.vue's app-scoped half. M2 stage B2b (2026-08-12, ledger row 2346, `.claude/dispatch-reports/lyt-boot-restoration.md`): SetupToolPalette moved OUT to its own new `A_setup` leaf.
│   │   ├── ToolbarEngineControls.vue  [B3]  M2 stage B2b boot-restoration wiring (2026-08-12, ledger row 2346, ruling row 2073): connect/disconnect + the engine-controls button cluster (mint-card/learn-path/play/match), extracted from the retired ToolbarEngineCluster.vue — mounted unconditionally at the `.lyt` encoding's own new `A_engine_controls` leaf (one of four independent leaves replacing the single `A_engine`). Finish-pass wave B2 (F2, `.claude/dispatch-reports/lyt-finish-pass.md` §3): realizes `lyt-capability-registry.ts`'s ratified per-class-point IR at the LIVE-MEASUREMENT granularity — `useEngineControlsRealization` decides, per its own current column width and current button widths, whether the cluster fits its compiled 80px reservation; below fit it switches to a `menu-path` trigger+menu (via `useClickTogglePopover`) so Connect stays reachable at 1280x1024/420x880 instead of clipping.
│   │   ├── ToolbarEngineMetrics.vue   [B3]  Live engine-telemetry strip leaf (version/winrate/scoreLead/PPS/latency/watchdog + mounts EngineModelSelect); self-sources the per-packet/per-tick reads, extracted out of the (now-retired) Toolbar.vue (render-coupling fix). MODEL slot moved to EngineModelSelect.vue (2026-08-06) so this leaf's own 1Hz tick re-render can't reach the select's DOM. M2 stage B2b (2026-08-12, ledger row 2346): now `group`-parameterised (`'eval'` = identity+winrate/scoreLead, `'health'` = pps/latency/watchdog), one instance per group mounted at the `.lyt` encoding's own new `A_engine_eval`/`A_engine_health` leaves — the retired ToolbarEngineCluster.vue's single `A_engine` mount is gone; queue moved OUT to EngineQueueTooltip.vue mounted directly at `A_engine_queue`.
...
│   │   ├── AppConfirmDialog.vue       [B1]  Sanctioned in-app replacement for `window.confirm`/`alert` (ADR-0019 S14), mounted once at App.vue; renders `useAppDialogs.ts`'s `currentDialogRequest` when `kind === 'confirm'`. `alert()` is the same shape with `cancelLabel: null` (single OK button). `useModalKeyboard`-wired.
│   │   ├── AppPromptDialog.vue        [B1]  Sanctioned in-app replacement for `window.prompt` (ADR-0019 S14), mounted once at App.vue; renders `useAppDialogs.ts`'s `currentDialogRequest` when `kind === 'prompt'`. Resolves to the typed string (possibly empty) or `null` on cancel — same three-way outcome native `prompt()` gave the 6 renamed call sites. `useModalKeyboard`-wired.
...
│       ├── WizardStepDemoBoard.vue    [B3]  Step (d), the centerpiece — hydrates the demo asset via `useSetupWizardDemoBoard`, mounts the real `BoardWidget` (ADR-0010: reuse, never fork), and exposes the three display-toggle checkboxes (`showMoveSuggestions`, `overlayLayers.ownership.{liveness,continuous}`) plus the first five scalar knobs by ascending registry priority (`display.move-filter-threshold` / `.ownership-opacity-ceiling` / `.ownership-deadband-threshold` / `.liveness-threshold` / `.hue-offset`, verified against `store/defaults.ts`) via `KnobSlider`. Folds in step (e) — the move-filter slider is priority-0, already first of the five.
│       ├── WizardStepPvAnimation.vue  [B3]  Step (f) — cycles the three `usePvAnimation` modes (instant/sequential/window) plus the annotation (numbering) setting, naming the relevant timing knobs per mode. Writes `session.ui.pvAnimation` directly (same cell every real board reads); the live preview drives its own `usePvAnimation` instance against the demo board's captured top-move PV — see the file's header for why this doesn't reuse `MoveSuggestions.vue`'s hover-driven trigger.
│       ├── PvAnimationPreview.vue     [B1]  Animated half of the PV step, isolated leaf (ADR-0010, row 748/749 defect 5): owns displayStones render + mode switcher + auto-advance interval so animation frames never re-render the parent's annotation select (the UI#1 flicker class).
│       ├── WizardStepSgfImport.vue    [B1]  Step (g), optional — mounts the existing `LibraryImportPanel` + `useLibraryImport` verbatim (same `/library/games/import` wire), no second import pipeline.
│       └── WizardStepFinish.vue       [B1]  Read-only summary of the cells the wizard touched, read straight from the live store — Settings shows the identical values immediately after.
│
├── composables/                             Logic layer. Pure-ish functions over reactive refs.
│   ├── useAppDialogs.ts               [B1]  Promise-based `confirm()`/`alert()`/`prompt()` replacing the 17 native call sites (ADR-0019 S14). Module-scoped `currentDialogRequest` signal (`ConfirmRequest | PromptRequest | null`) carrying its own `resolve` callback, same shape as `useSetupWizardSignal.ts` / `useMintDialogSignal.ts`; consumed by `AppConfirmDialog.vue` / `AppPromptDialog.vue`.
...
│   ├── useModalKeyboard.ts            [B1]  Shared modal keyboard/focus mechanism (ADR-0019 S5 fix): Escape→close, Tab/Shift+Tab focus trap, initial focus, focus restoration; exports `anyModalOpen` for useUserIORegistry's global-hotkey suppression while a modal is open. Originally wired into 7 src/components/modals/*.vue files at S5; now 9 (ADR-0019 S14 added AppConfirmDialog.vue / AppPromptDialog.vue), plus SetupWizardModal.vue.
│   ├── useNavigation.ts               [B2]  Headless navigation within the game tree (next/prev/parent/child/toggle-main-line).
│   ├── useProxyUpstreamSetting.ts     [B1]  Desktop-only (ledger rows 860-862) in-app editor for the bundled proxy sidecar's upstream WebSocket URL — the cell components/ProxyUpstreamSettingField.vue (mounted by both WizardStepEngineUri.vue and chrome/SettingsPane.vue) reads/writes, via Tauri commands `get_proxy_upstream_setting`/`set_proxy_upstream_setting` (src-tauri/src/proxy_settings.rs). Validated by lib/ws-url.ts (shared with useEngineUriEditor.ts). Inert (no-op, never invokes) outside Tauri — gated on config/env.ts's `IS_TAURI`. Takes effect on next app launch, not live.
...
│   ├── useSetupWizard.ts              [B1]  First-run setup wizard's step machine (ledger slug swz-setup-wizard): current-step navigation only (next/back/skip/goTo/finish). Every step writes its own real store cell directly, so skip and next are behaviourally identical — see the file's header. `finish()` is the sole writer of `profile.settings.onboarding.completed`.
│   ├── useSetupWizardDemoBoard.ts     [B3]  Module-memoised demo-board hydration for the wizard's centerpiece step: loads `lib/setup-wizard-demo-loader.ts`, seeds `state/analysis-ledger.ts` under the SAME key `state/analysis-config.ts::activeAnalysisKeys` resolves live (so the real `BoardWidget` overlays pick it up with no second pipeline), and exposes the captured top move's PV for the PV-animation step.
│   ├── useSetupWizardSignal.ts        [B1]  Module-scoped "setup wizard is open" boolean signal (`setupWizardOpen` / `openSetupWizard()` / `closeSetupWizard()`), same shape as `captureMode` / `useModalKeyboard`'s `openModalCount`. App.vue's first-run watcher and Settings' "Re-run setup wizard" button both open through this one entry point.
...
│   │   ├── useIntervalSummary.ts      [B3]  Per-colour summary row for the current selectionRange; projects the ply range onto each colour's local move-range (useTriangularHeatmap's own `plyRangeToColorMoveRange`) and looks the cell up in its matrix — shared kernel, not a recompute.
...
│   │   ├── useTriangularHeatmap.ts    [B3]  Extracts proxy-side triangular heatmap from the ledger for a path; sole authority for the PlyIndex ↔ ColorMoveIndex conversion (colorMoveToPly + its reverse plyRangeToColorMoveRange).
...
│   │   ├── ghost-stone.ts             [B3]  wiki2-ghost-stone: pure `computeGhostStone(enabled, turn, hoverPoint)` — the whole hover-preview decision (visible/where/color), deliberately no BoardState param so it structurally cannot gate on legality/occupancy/capture. Called by BoardDisplay.vue against its own pointer-tracked hover position.
...
│   │   ├── useBoardMoveRouting.ts     [B3]  Grading-integrity gate for board-mutation entry points (click-to-play, pass, paste-PV): AWAITING_MOVE routes to the review session's graded handler (processUserMove / processUserPass), transient SR states refuse mutation, REVIEWED (a restored deck-repeat snapshot) refuses outright, free play (with the game-head engine trigger) is IDLE/FINISHED-only. Extracted from App.vue 2026-06-11; `handlePass` added for pass-support (2026-08-06).
...
│   │   ├── useKnownPositions.ts       [B1]  Fetch/mutation API over the known-positions state module: the mint-time duplicate-position check (POST /positions/hash + lookup), append-on-mint, and hydrateKnownPositions (bulk boot-time fetch via GET /cards/hashes); state owned by state/known-positions.ts.
│   │   ├── useNodePositionHashes.ts   [B2]  card-position-annotations Stage B: viewport-driven, debounced (150ms) batch fill of the node-position-hashes cache via POST /positions/hash-batch; failure surfaces one non-blocking notice, never a stale/partial cache write. Called from TreeWidget's own nodeList watcher.
...
│   │   ├── useLytActivityInvariance.ts [B1]  METAMODEL WAVE item 2c, ported M2 stage F1 (ledger row 2311 disposition 2, verbatim from the model-iteration loop experiment): `checkActivityInvarianceL6` — a pure, directly-testable drift check between a leaf's declared compiled `envelopeStates` and its widget's registry-declared `activityStates` (L6, honestly narrower than a "missing envelope is a violation" gate — see the file's own header). NOT wired into App.vue's boot path this wave; exercised by tests/unit/lyt-activity-invariance.test.ts.
│   │   ├── useLytPresenceMenu.ts      [B1]  W2 (roadmap §8 W2 item 1, ledger row 1743): corner presence-menu logic — the target list + defaults, guarded toggle (last-remaining-panel refusal), railStyle read/write. Chrome-generic (no Go/game-tree vocabulary), matching LytNode.vue/LytPresenceMenu.vue's own [B1] tag. Presence arc P2b (`.claude/dispatch-reports/lyt-p2b-presence-realization.md`): a 4th target, `A_setup`, joins the original three; an optional `classDefaults` constructor option lets a real caller (LytPresenceMenu.vue, wired from App.vue) override the static per-target fallback with the ACTIVE screen class's own compiled default — `controlPanel`'s own default genuinely varies by class (repetition-first portrait).
│   │   ├── useLytOverflowCss.ts       [B1]  REALIZATION WAVE (item 3, lyt-realization-exclusive-overflow, row 1937): pure `LytLeafNode -> overflow CSS` mapping (`scrollAxes` -> `overflow-x`/`overflow-y: auto`), the Vue-runtime analog of useLytTrackCss.ts's own `LytTrackShape -> CSS` mapping. No Vue reactivity — `leafOverflowStyle(node)` is a plain `(node) -> style object`, unit-testable directly.
│   │   ├── useContentDemand.ts        [B1]  Space-owner cure, dispatch L2b (ledger rows 2447/2450/2460, `.claude/dispatch-reports/lyt-space-owner-spec.md` §2's runtime-content-dependent-demand seam): `useContentDemand(el, axis) -> Ref<Px|null>`, one `ResizeObserver` per measured element reading `scrollWidth`/`scrollHeight` (never the currently-allotted box), refused loudly on an unknown axis or a still-null element ref past mount. `TreeWidget.vue` wires it on its own `outerRef` (axis `'h'`, the `tree` leaf's own compiled axis in both classes) and exposes the resulting ref via `defineExpose`; feeding it into a live `FeasibleLayout` overlay is step 3's own scope, not this build's.
│   │   ├── useLytFitAssertion.ts      [B1]  Space-owner cure, dispatch L2b: the mount-time/geometry-transition Fit assertion (spec §3 step 2's own "Gate") — one shared `ResizeObserver` per `LytNode.vue` instance checking every Fit-disciplined leaf/blackbox cell (`content: 'bounded'|'designed'`) for `scrollHeight<=clientHeight && scrollWidth<=clientWidth`, pushing a structured `pushSystemMessage('error', ...)` naming the region and overflow amounts on violation. Report-only — no clamp, no layout write. Wired into `LytNode.vue`'s own leaf-cell function ref (`onLeafCellRef`).
│   │   ├── useLytProgramIndex.ts      [B1]  LYT R1 (`.claude/dispatch-reports/lyt-r1-orientation-pathmap.md`, PART 2, ADR-0011 Rule 2 trigger row 2345): `buildLytProgramIndex` walks a compiled `LytProgram` once, building a `widget id -> path` / `widget id -> leaf node` index — App.vue derives its DOM-id map, resizer-drag override keys, and the `tree` leaf's `orientation` fact off this instead of hand-copied dotted-path literals. `lytParentPath` (a Split/Exclusive node's own path from a child's) and `lytOrientationToProp` (the `LytAxis` -> TreeWidget `orientation` prop mapping) are the two small pure helpers alongside it. No Vue reactivity — every export is a plain function, unit-testable directly.
│   │   ├── useLytTrackCss.ts          [B1]  W1 skeleton rework (ledger row 1781): pure `LytTrackShape -> CSS Grid track value` mapping (fixed/elastic/elastic-capped/board-priority-clamp), extracted out of LytNode.vue per ADR-0007. No Vue reactivity — every function is `(shape) -> string`, unit-testable directly. W2 (ledger row 1743) generalizes `trackCssValue`'s board-priority-clamp branch with an optional `leadingReservedPx` param (boardRail's own live width) — additive, defaults to 0 (byte-identical pre-W2 output).
│   │   ├── usePopoverEdgeClamp.ts     [B1]  Viewport-edge clamp for hover popovers (setPopoverEl function-ref + xShift) — translateX shifts the rendered popover inward when its CSS anchor would push it off-screen. Consumers: BoardRailPopoverTrigger.vue, LytPresenceMenu.vue, and (presence arc P2b) App.vue's own control-panel popover summon trigger (all three in `#lyt-corner-chrome`, outside `.lyt-toolbar-strip`'s clip, so `position: absolute` + `bottom: 100%` is the right shape for them — `useFixedAnchoredPopover` was tried for the third consumer first, per that arc's own commission naming, and empirically failed at this exact corner position; see App.vue's own script-header comment for the witnessed defect).
│   │   ├── useFixedAnchoredPopover.ts [B1]  Commission lyt-popover-clip-class (work item row 1968): `position: fixed` clip-ancestor-escape for toolbar-hosted popovers mounted inside `.lyt-toolbar-strip` (`overflow-y: auto`) — trigger-anchored top/left, scroll+resize re-anchor, horizontal+vertical viewport clamps. Extracted from ToolbarSliderPopover.vue's D1 fix; consumers: ToolbarSliderPopover.vue, EngineQueueTooltip.vue, PboPopover.vue, LocalePicker.vue (row 1984 — the first click-toggled, not hover-driven, consumer; the `open: Ref<boolean>` contract is interaction-agnostic and composed unmodified), ToolbarEngineControls.vue (finish-pass wave B2, via useClickTogglePopover.ts).
│   │   ├── useClickTogglePopover.ts   [B1]  Finish-pass wave B2 (ToolbarEngineControls.vue's new menu-path realization, F2): generalizes the click/outside-click/Escape dismiss + `useFixedAnchoredPopover` position shape LocalePicker.vue/LytPresenceMenu.vue each hand-author inline — a third near-identical copy crossing the ADR-0012 P1 "one home per fact" threshold. Deliberately NOT retrofitted into those two pre-existing components (out of that commission's own scope).
│   │   ├── useEngineControlsRealization.ts [B1]  Finish-pass wave B2 (F2 + W-B1's adjacent 420px finding — `.claude/dispatch-reports/lyt-finish-pass.md` §3, `.claude/dispatch-reports/lyt-wB1-portrait-priority.md` §STOP-and-report item 3): Vue wiring for `state/engine-controls-realization.ts`'s pure decision — `useElementWidth` (ADR-0012 P1: one home per fact) for the live column width, plus a permanently-mounted hidden shadow clone of the five real buttons (carrying the SAME reactive labels) for the live button widths, so the realization decision reflects the ACTUAL current rendered content rather than a hand-maintained worst-case table.
...
│   │   ├── useSettingsSubTab.ts       [B1]  Work item `lyt-settings-live-opening` (ledger rows 2007/2009/2001): the settings sub-tab active-id + `{id, label}[]` table, shared between the two separately-mounted `SettingsSubstrip.vue`/`SettingsPane.vue` instances via a module-singleton ref (not a persisted store field — see the file's own header for why).
...
│   │   ├── useSystemLogToggle.ts      [B1]  D2 fix (`.claude/dispatch-reports/lyt-w5-parity-build.md` Defect D2): thin read/toggle over `session.ui.systemLogExpanded` + `touchSession()` — the write side of the restored system-log manual affordance (SystemLogToggle.vue is the button). No guard logic (unlike useLytPresenceMenu.ts's three targets): hiding the log never leaves the user without a visible panel.
...
│   │   ├── closeAtScale.ts            [B2]  closeAtScaleScenario — the `close-at-scale` scenario: build ~230 boards (each forwarded to move 50) via setUpManyBoards, then close every board in the heaviest order (front/active board, one per frame) while the trace records. The board-count-scaling close-path profile, an order of magnitude past the 16-board studied regime; runs no analysis. Emits setup / presettle / closeall (+ per-close `close`) / drain marks. Builds its own rail, manages its own cleanup.
...
│   │   ├── jankSubstrate.ts           [B2]  Shared docked-thumbnail-jank substrate (extracted from useJankTest 2026-06-12): fixed Shusaku metadata lookup (fail-loud if absent), 16-board rail setup (setUpRail), ~230-board scale rail (setUpManyBoards — cycles fetched bodies, bounded HTTP), forward-to-ply nav, synthetic hover-scrub stimulus. Consumed by useJankTest (toggle), jankExtended (scenario), and closeAtScale (scenario).
...
│   ├── known-positions.ts             [B1]  Per-user reactive `ContentHash -> CardId` map (card-position-annotations Stage A); populated incidentally via BackendService.mapToReviewCard AND in bulk at boot/re-auth via useKnownPositions.hydrateKnownPositions (GET /cards/hashes), first-seen-wins on write, purged on identity flip.
│   ├── lyt-layout.gen.ts              [B1]  GENERATED (research/lyt/emit_layout_tree.py) — the compiled LYT landscape program (H/V/Exclusive tree, unsolved) as typed TS data, consumed at RUNTIME by LytNode.vue (W1 skeleton rework, ledger row 1781; roadmap §3 "layout as data, not template"). Do not hand-edit.
│   ├── lyt-layout-portrait.gen.ts     [B1]  GENERATED (research/lyt/emit_layout_tree.py --registration portrait, from research/lyt/encodings/lengyue_portrait.lyt, layout "lengyue-portrait") — the compiled LYT portrait program (H/V/Exclusive tree, unsolved) as typed TS data, consumed at RUNTIME by LytNode.vue (W3 resizers + screen-class swap, roadmap §8 W3). Discloses the same simplification as its landscape sibling (the control-panel Exclusive node collapsed to one blackbox leaf rather than its five CP-* children). Do not hand-edit.
│   ├── lyt-layout-types.ts            [B1]  HAND-WRITTEN (W3, ADR-0012 one-home-per-fact): the `LytProgram` type vocabulary extracted out of `lyt-layout.gen.ts`/`lyt-layout-portrait.gen.ts`, where it used to be duplicated byte-for-byte inline in both. Both generated files now `export type * from './lyt-layout-types'` to re-publish it under their existing import paths, so no consumer (LytNode.vue, useLytTrackCss.ts, useLytPresenceMenu.ts, lyt-widget-registry.ts, store/defaults.ts, App.vue) needed an import-path edit. M2 stage F1 (ledger row 2311 disposition 2): `LytLeafNode` gains eight inert realization-layer metadata fields (`elasticAxes`/`ceilingAxes`/`floorAxes`/`edgeAxes`/`orientation`/`activity`/`demote`/`envelopeStates`), ported from the model-iteration loop experiment — no current consumer reads them; the experiment's `unitAxes`/`wrapPolicy` fields and two track-shape algorithm changes are a disclosed narrowing, NOT ported this stage (see emit_layout_tree.py's own module docstring for why).
│   ├── lyt-widget-registry.ts         [B1]  W1 skeleton rework (ledger row 1781): hand-authored leaf-widget-id -> mount-disposition table (`mounted`/`absorbed`/`absent`) LytNode.vue and App.vue's slot wiring both consult. The DATA half of the "why slots, not a live component table" contract; see the file's own header. W2 (ledger row 1743): boardRail now actually mounts SidebarWidget (was inert, presenceDefaultVisible false with no menu to flip it on); previewBoard flips 'absent' -> 'mounted' (PreviewBoardPanel.vue). METAMODEL WAVE item 2b, ported M2 stage F1 (ledger row 2311 disposition 2): each entry gains an `activityStates` column (`readonly string[] | null`), populated only on the five widget ids the F1 port verified directly against component source (B/I_board/tree/CP-analysis/A_board) — consumed by useLytActivityInvariance.ts's L6 checker. M2 stage B2b BOOT-RESTORATION WIRING (2026-08-12, ledger row 2346, `.claude/dispatch-reports/lyt-boot-restoration.md`): registers the widget ids M2 stage B2b's own encoding edit added to the compiled programs but left unregistered (a full-app-crash defect this commission fixes) — `A_engine` retired into `A_engine_controls`/`_eval`/`_health`/`_queue` (ToolbarEngineControls.vue / ToolbarEngineMetrics.vue×2 / EngineQueueTooltip.vue), `A_setup` added (SetupToolPalette.vue; its then-disclosed always-visible App.vue-level override was RETIRED by presence arc P2b — `A_setup` is now a genuine 4th `useLytPresenceMenu.ts` target, default off, user-choice sovereign, per that arc's own dispatch report), `settingsPane` renamed `SP_session` (SettingsPane.vue, unchanged disposition).
│   ├── lyt-capability-registry.ts     [B1]  METAMODEL WAVE item 4, ported M2 stage F1 (ledger row 2311 disposition 2, verbatim from the model-iteration loop experiment): a typed action-capability IR (`LYT_CAPABILITY_REGISTRY`/`LYT_CAPABILITY_REALIZATION`) over the toolbar's action elements — DATA ONLY, no live wiring this wave (petrification guard in the file's own header). `ToolbarEngineControls.vue`/`ToolbarAppCluster.vue` are unchanged by this file's presence (M2 stage B2b, 2026-08-12: comment references updated for ToolbarEngineCluster.vue's retirement; no data/behaviour change).
│   ├── engine-controls-realization.ts [B1]  Finish-pass wave B2 (F2 + W-B1's adjacent 420px finding — see `composables/chrome/useEngineControlsRealization.ts`'s own header for the full citation chain): pure `button-cluster`/`menu-path` realization decision for `A_engine_controls`'s five capabilities — `computeWrappedRowCount` (CSS flex-wrap line-assignment simulation), `computeClusterNeededHeightPx`, `resolveEngineControlsRealization`, plus the compiled program's own `80px` reservation constant and a live-measured worst-case button-width table (documentation/testing only — NOT the runtime mechanism, which measures the real current buttons instead).
│   ├── feasible-layout.ts             [B1]  Space-owner cure, dispatch L1 (ledger rows 2446/2447, `.claude/dispatch-reports/lyt-space-owner-spec.md` §3 step 1) — the additive-only validation layer. `Measured<Region>`/`measured()` (a region's own branded `Px` min/preferred/maxUseful triple, refused loudly on self-contradiction), `FeasibleLayout.validate()` (whole-assignment starved/hoarding validator, private-constructor + refusal-as-value), `RegionPresence` (presence-as-zero-allotment doc), `SovereignOverride`/`resolveSovereignOverrides` (drag exemption + casualty diagnostic, real threaded screenClassId/viewport signature vs. the spec's own elided sketch), and the `measuredFromLytProgram` adapter reading every track-bearing leaf/blackbox/exclusive of a compiled `LytProgram` into `Measured` entries (board aspect-coupled leaves and untracked Exclusive-tab children excluded, disclosed). No existing runtime behavior changes — `layout-model.ts`'s clamps and `LytNode.vue`'s realization are untouched by this step. Dispatch L2b (ledger rows 2447/2450/2460): `measuredFromLytProgram` gains an optional `overlay: ReadonlyMap<string, Px|null>` param (`useContentDemand.ts`'s own runtime readings) — supersedes a synthesized-null `maxUseful` only (never a real compiled ceiling), clamped to `max(min, overlayPx)`; the literal spec gate (`content === 'unbounded'`) is narrowed to "synthesized maxUseful is null" since `tree`'s own compiled `content` is still `null` in both programs (a disclosed, frontend-scope-respecting divergence — see the module's own header).
│   ├── layout-model.ts                [B1]  Resolution-roadmap Phases 0+1: mints `LayoutClass` (`axis: 'row'|'column'` from aspect ratio, `width: 'compact'|'standard'|'wide'|'vast'`) and the panel-geometry floors keyed by it (`getPanelGeometryPolicy`) — single home for what used to be hand-picked pixel literals in `useResizablePanel.ts` (`CONTROL_PANEL_MIN_WIDTH_PX`, audit finding R2) and `ForestDirectory.vue` (the 479px narrow-stack threshold). `useDeferredLayoutClass` is the reactive entry point (App.vue's `#split-workspace` axis flip), hysteresis-and-drag-deferred like `useDeferredContainerBreakpoint.ts`. lyt-constants-swap (ledger row 1687): `RESIZER_WIDTH_PX` is now sourced from `lyt-solved-layout-asis.gen.ts` rather than a hand literal — the one panel constant the LYT shadow-harness's divergence report showed the as-is solve and the live DOM agreeing on exactly.
│   ├── lyt-solved-layout-asis.gen.ts  [B1]  GENERATED (research/lyt/emit_ts.py --registration current_row_asis.lyt) — solved rectangles for the LYT as-is conformance baseline (research/lyt/fixtures/transcription/current_row_asis.lyt) at each representative screen size. No longer shadow-only: `layout-model.ts` imports `RESIZER_WIDTH_PX` from it (lyt-constants-swap, ledger row 1687). Do not hand-edit.
│   ├── lyt-solved-layout-landscape.gen.ts [B1]  GENERATED (research/lyt/emit_ts.py --registration "lengyue_landscape+portrait" --class-id landscape) — solved rectangles for the W1-realized landscape program (research/lyt/encodings/lengyue_landscape.lyt) at each representative screen size, presence-pruned against the registration's own default valuation. Consumer: frontend/scripts/lyt-conformance.mjs --source landscape. Do not hand-edit.
│   ├── lyt-solved-layout.gen.ts       [B1]  GENERATED (research/lyt/emit_ts.py, default registration) — solved rectangles for the LYT-repaired encoding (research/lyt/fixtures/transcription/current_row_repaired.lyt) at each representative screen size. Phase-1 shadow mode: no runtime import from app code (frontend/scripts/lyt-conformance.mjs is the only consumer). Do not hand-edit.
│   ├── node-position-hashes.ts        [B2]  Per-node reactive `NodeId -> ContentHash` cache (card-position-annotations Stage B); mirrors thumbnail-render-resources.ts's invalidation shape (purge-on-board-close, purge-on-identity-flip). Filled by composables/cards/useNodePositionHashes.ts.
...
│   ├── archived-migrations.ts         [B1]  Aged-out schema migrations (1→2 .. 73→74, per the file's own scope header) lifted out under the rolling-archive cadence to keep migrations.ts scoped to the latest two; preserved for the framework's contiguity invariant. Post-retrofit bodies call witnessedContainer (imported from migration-witness.ts).
...
│   ├── teardown-registrations.ts      [B3]  Bootstrap: side-effect-imports every resource-owner module so its board-close / workspace-reset teardown handler registers before any closeBoard / resetWorkspace fires (ADR-0012 P2/P3 load-guarantee). Manual list, tripwired by the board-completeness test. Imported early from main.ts. B3 because it value-imports the (B3) owner modules wholesale — a pure wiring shim whose band tracks its highest dependency, like the types.ts barrel / store index.
│   └── teardown-registry.ts           [B1]  Leaf registry for board-close / workspace-reset teardown handlers (ADR-0012 P2/P3 dependency inversion) — owners register; closeBoard / resetWorkspace run them via the registry instead of importing the owners (the store → owner out-edge removal that broke the import cycle). Imports only types; keep it a leaf.
│
...
│   ├── setup-wizard-demo-loader.ts    [B3]  First-run setup wizard's demo-board hydration (ledger slug swz-setup-wizard): loads + runtime-validates `assets/setup-wizard-demo.json` (loud failure on a malformed asset, never a blank board), replays its recorded move list through `logic.ts`'s pure rules engine (`applyGoMove`/`applyPass` via the new `engine/util.ts::fromGtp`) to a real BoardState, and returns the captured KataGo analysis packet unmodified for the ledger to consume. Zero network, zero engine.
