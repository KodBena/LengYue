...
│   │   ├── scenarios.ts               [B2]  Scenario registry + built-ins (nav-only / nav-range / stability-nav-range / full-stress / workspace-reset; analysis preamble connects engine + cold-cache, protocol defaults 1000 visits / no-adapt / SELECTOR) + window.__perfScenario (run / list / disconnect) install. stability-nav-range pins the Stability sub-tab so StabilityPanel/useStabilityMetrics run under packet load (the other scenarios pin Basic, which omits it).
