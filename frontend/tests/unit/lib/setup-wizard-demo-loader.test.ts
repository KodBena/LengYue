...
function validAsset(overrides: Record<string, unknown> = {}) {
  return {
    provenance: {
      gameRawId: 1, black: 'B', white: 'W', date: '2020-01-01',
      result: 'B+1.5', position: 2, rules: 'japanese', komi: 6.5,
      model: 'test-model', maxVisits: 100,
    },
    moves: [['B', 'Q16'], ['W', 'D4']],
    analysis: {
...
describe('loadSetupWizardDemo — happy path (real bundled asset)', () => {
  it('loads and replays the real asset to a real BoardState', () => {
    ...
    expect(demo.provenance.black).toBe('Go Seigen');
    expect(demo.provenance.white).toBe('Fujisawa Hosai');
  });
