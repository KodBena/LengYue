...
describe('useIntervalSummary — shared-kernel equality with the multiresolution heatmap', () => {
  it('resolves to the exact HeatmapCell useTriangularHeatmap produces for the projected interval', () => {
    ...
    // Exact object equality (not just `.value` equality): proves the
    // summary is READING the heatmap's own cell record, not deriving an
    // independently-shaped one that happens to carry the same number.
