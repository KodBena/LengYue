...
describe('useMoveDeltaAnnotation — found (both halves in the ledger)', () => {
  ...
  // Review compose step (wf7-child-delta-overlay-review.md finding 1):
  // colour-local index ≥ 1 — a deep Black move with an intervening White
  // move must index deltaSeries.black at 1, not 0 (the ply → colour-local
  // arithmetic, exercised past the trivial first move).
  it("a deep Black move (after an intervening White move) indexes deltaSeries.black at colour-local index 1", () => {
    const { boardId, board } = setup('(;FF[4]GM[1]SZ[19];B[pd];W[dp];B[pp])');
    const childId = navigateToPathIndex(boardId, 3); // B[pp] — Black's move 1

    const { rawKey, enrichedKey } = activeAnalysisKeys.value;
    ledger.recordRaw(rawKey, childId, rawAt(220));
    // Index 0 carries a decoy value: a wrong colour-local index would
    // surface 9.99 instead of the expected 0.31.
    ledger.recordEnrichment(enrichedKey, childId, { black: { deltas: { '0': 9.99, '1': 0.31 } } });

    const annotation = withSetup(() => useMoveDeltaAnnotation(() => board, () => childId));
    expect(annotation.value?.color).toBe('B');
    expect(annotation.value?.visits).toBe(220);
    expect(annotation.value?.delta).toBeCloseTo(0.31);
  });
...
describe('useMoveDeltaAnnotation — absence (never a zero placeholder)', () => {
  ...
  // Review compose step (wf7-child-delta-overlay-review.md finding 1):
  // a pass child consumes a path slot but has no point to annotate —
  // the overlay must render nothing, even when the ledger holds data
  // for the node.
  it('is null for a pass-move child (no point to annotate), even with ledger data present', () => {
    const { boardId, board } = setup('(;FF[4]GM[1]SZ[19];B[pd];W[])');
    const childId = navigateToPathIndex(boardId, 2); // W[] — a pass

    const { rawKey, enrichedKey } = activeAnalysisKeys.value;
    ledger.recordRaw(rawKey, childId, rawAt(80));
    ledger.recordEnrichment(enrichedKey, childId, { white: { deltas: { '0': -0.2 } } });
