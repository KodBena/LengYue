...
| `GameDisplayOrdinal` | number | server-computed, per-user counter | ACL re-brand at `library-service.ts::fromWireGameListItem`/`fromWireGame` (`wire.display_ordinal as GameDisplayOrdinal`) | persisted (`game_source.display_ordinal`) | small, per-user (starts at 1) | Sound; two ACL sites (list + detail projections). Display site: `LibraryTable.vue`'s `#` column. |
| `CardPublicId` | string (UUID) | server-minted at card-creation | ACL re-brand at `backend-service.ts::mapToReviewCard` (`raw.public_id as CardPublicId`) | persisted (`card.public_id`, unique) | large (100s–1000s in a real deck) | Sound; single ACL site. Reference-role opaque handle, `card`'s sibling of `GameSourceId`'s `client_game_id`/`BoardId` counterpart — not yet consumed as a request-addressing value anywhere in the SPA (carried for forward use; the design defers the full PK→public_id addressing swap as a named follow-on). |
