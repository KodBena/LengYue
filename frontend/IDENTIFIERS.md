...
| `BookmarkId` | string | client UUID | `useQeubo.ts:842` (`generateUUID() as BookmarkId`); single construction site | per-session (qEUBO/PBO bookmarks; pinned ones persist to `qeuboPinnedBookmarks`) | ~0–dozens | Sound; single site. |
...
| Name | Prim. | Origin | Construction | Lifetime | Cardinality | Status / notes |
|------|-------|--------|-------------|----------|-------------|----------------|
| `CardId` | number | server PK | ACL re-brand at `backend-service.ts` (`raw.id as CardId`, etc.) | persisted (DB row id; flows in synced docs and the card forest) | large (100s–1000s in a real deck) | Sound at the ACL. **Brand-stripped downstream** past the ACL via `as unknown as number`: `useMinting.ts:50` (justified, `:42`), `useCardTreeData.ts:282,452,482`, `useCardTreeHydration.ts:44`, `card-tree-echarts.ts:249`, `ForestDirectory.vue:237`. See `[leaky]` erosion (b). **Per-user-id-enumeration design** (`.claude/dispatch-reports/per-user-id-enumeration-design.md`): `id`/`CardId` stays the raw PK — a named allowlist exception alongside `GET /cards/{card_id}`'s path param — because it's the addressing value every already-fetched, tenant-scoped card round-trips through (PATCH, review, card-tree parent linking, `known-positions.ts`'s `ContentHash → CardId` map). `CardDisplayOrdinal` (below) is the non-leaking sibling for anything a user *reads*. |
| `GameSourceId` | number | server PK | ACL re-brand at `backend-service.ts` and `library-service.ts` (`wire.id as GameSourceId`) | persisted (DB row id) | ~10s–100s | Sound at the ACL. Same `as unknown as number` stripping at `ForestDirectory.vue:236`. See `[leaky]` erosion (b). Same per-user-id-enumeration framing as `CardId` above — still the addressing value for `/library/games/{id}`. **Browse-leak-fix** (ledger rows 417/423) closed the `/stats/forests` + `/lineage/*` deferred exception this row used to name: those endpoints no longer surface `GameSourceId` at all — `ForestStat`/`RootGroup`/`CardLineageTree` carry `GameDisplayOrdinal` instead. |
| `CardDisplayOrdinal` | number | server-computed, per-user counter | ACL re-brand at `backend-service.ts::mapToReviewCard` (`raw.display_ordinal as CardDisplayOrdinal`) | persisted (`card.display_ordinal`) | small, per-user (starts at 1) | Sound; single ACL site. Display-role only — never round-tripped as a reference; distinct brand from `CardId` prevents accidental interchange. Display sites: `card-tree-echarts.ts`'s on-canvas node label. |
| `GameDisplayOrdinal` | number | server-computed, per-user counter | ACL re-brand at `library-service.ts::fromWireGameListItem`/`fromWireGame` and `backend-service.ts::mapForestStat`/`mapResolvedRoot`/`fetchTreeByRoot` (`raw.display_ordinal`/`raw.game_source_display_ordinal as GameDisplayOrdinal`) | persisted (`game_source.display_ordinal`) | small, per-user (starts at 1) | Sound; multiple ACL sites (list + detail projections, plus the browse-leak-fix additions). Display sites: `LibraryTable.vue`'s `#` column, `ForestTreeNav.vue`'s game-id chip. **Browse-leak-fix** (ledger rows 417/423): also the `NavSelection`'s `{kind:'game'}` payload and `ForestNavGameNode.gameSourceId` — the per-user-scoped addressing value the Forest navigator now selects/groups by, replacing the raw `GameSourceId`. |
| `CardPublicId` | string (UUID) | server-minted at card-creation | ACL re-brand at `backend-service.ts::mapToReviewCard`/`mapForestStat`/`mapResolvedRoot`/`fetchTreeByRoot` (`raw.public_id`/`raw.root_card_public_id as CardPublicId`) | persisted (`card.public_id`, unique) | large (100s–1000s in a real deck) | Sound; multiple ACL sites. Reference-role opaque handle, `card`'s sibling of `GameSourceId`'s `client_game_id`/`BoardId` counterpart. **Browse-leak-fix** (ledger rows 417/423): now genuinely consumed as a request-addressing value — `POST /lineage/tree-by-root`'s `root_card_public_id` body field, `NavSelection`'s `{kind:'root'}` payload, `ForestNavRootNode.rootCardId`, `CardLineageTree.rootCardPublicId`, `RenderTree.rootCardId`, `CardTreeWidget`'s `forestStats` Map key and `collapse-tree` emit. Closes the "not yet consumed" gap this row used to name. |
...
| Name | Prim. | Origin | Construction | Lifetime | Cardinality | Status / notes |
|------|-------|--------|-------------|----------|-------------|----------------|
| `ContentHash` | string | server-computed SHA-256 (lowercase hex) | ACL re-brand at `backend-service.ts` (`mapToReviewCard`'s `raw.content_hash as ContentHash`; `hashPosition`'s `raw.content_hash as ContentHash`; Stage B's `hashPositionsBatch`'s `raw.content_hashes as ContentHash[]`) | per-session (known-positions map / node-position-hashes cache); not itself persisted client-side | one per distinct owned position (≤ card count) for known-positions; one per hashed tree node for node-position-hashes | Sound at the ACL — three construction sites (single, matches the wire's `content_hash: string`; batch, matches `content_hashes: string[]`), all re-branding backend output (`domain.card.Card._serialize_content_hash` / `PositionHashResponse.content_hash` / `PositionHashBatchResponse.content_hashes`). Consumed by `state/known-positions.ts`'s `Map<ContentHash, CardId>` and (Stage B) `state/node-position-hashes.ts`'s `Map<NodeId, ContentHash>`. |

### Local ids (board-scoped, not server-issued)

| Name | Prim. | Origin | Construction | Lifetime | Cardinality | Status / notes |
|------|-------|--------|-------------|----------|-------------|----------------|
| `NodeId` | string | local id — `'root-'+short` / `'node-'+short` (short = `Math.random().toString(36)`, **not** a UUID) | factory `asNodeId` (`board-factory.ts:21`); roots at `board-factory.ts:50` (`asNodeId('root-'+uuid())`); SGF-loaded nodes at `sgf-loader.ts:74` (justified single-site brand mint `('node-'+uuid()) as NodeId`, threaded through `transform`/`hydrate` — replaced the three bare `as any` at the former `:76,77,89` in the 2026-06-10 cast-hygiene arc); a fresh node in `logic.ts:116` (`'node-'+Math.random()… as NodeId`); ACL re-brand at `analysis-persistence-service.ts:163` | per-session board-scoped; **some persist** as ledger / trajectory composite keys (see "representation cost" below) | **Highest in the system**: one per game-tree node, ~340–1000+ per board × N boards | **`[leaky]`** — the soft underbelly. **27** raw `rg "as NodeId" src` line matches (re-measured 2026-06-10 after the branded-path-types arc retired 14 redundant element/array re-casts at path consumers; 8 of the 27 are prose mentions in comments, not casts, and 2 are the vacuous empty-path mints in `useVariationPath.ts:29,45`). Two distinct causes, only one self-inflicted: (1) `Object.keys(board.nodes)` returns `string[]` though `board.nodes` is `Record<NodeId, GameNode>` (`src/types/game.ts:98`) — a TypeScript limitation, the cast is unavoidable and the `useActivePath.ts:19-23` comment names it the "Category C" boundary; (2) genuine self-inflicted widening, e.g. `useActivePath.ts:14-15` declares `path: string[]` / `currId: string` instead of `NodeId`, forcing the re-brand at `:24` (the former `useReviewSession.ts` examples were retired by the branded-path-types arc). Two generators mean **NodeId is not UUID-shaped**, so any consumer assuming a `'\|'`-free UUID form is on thin ice (`stability-trajectory-store.ts` embeds it in a `\|`-delimited key — safe only because the short form has no `\|`). See erosion (a). |
...
| `BranchRangeKey` [B2] | string | composite: the ordered sequence of `${decisionNodeId}:${chosenChildId}` pairs (`|`-joined) at every node along the active `RootToLeafPath` with more than one child — `decisionSequence` [tree-positional] is the sole leg (no Go content, no palette/config dependence; band-called per the keyed-cache rule in `frontend/CLAUDE.md` "Type-driven design"). A single-child node contributes nothing, so the key is stable across ordinary forward play and changes only when an ancestor's `activeChildIndex` genuinely diverges (a branch switch) | sole factory `deriveBranchRangeKey` (`composables/analysis/branch-range-key.ts`) | per-board, persisted as the key-space of `BoardState.analysisRanges` (design proposal §1 Candidate C; commissioner adjudication, ledger rows 112/119 — the map is deliberately UNCAPPED, no LRU) | unbounded per board (one per distinct branch stem a user has visited; growth is bounded in practice by session-visited branches, not enforced) | Sound; single factory. Legacy-blob carry-over: schema migration 61 → 62 (`store/migrations.ts`) computes this same key from a frozen blob's current active path to convert a pre-existing single-slot `analysisRange` — that computation is intentionally inlined there (frozen migration body) rather than importing this factory. |
...
| Name | Prim. | Origin | Construction | Lifetime | Cardinality | Status / notes |
|------|-------|--------|-------------|----------|-------------|----------------|
| `ColorMoveIndex` | number | derived index — 0-indexed within one colour's move sequence (native to KataProxy's triangular heatmap) | `composables/analysis/useTriangularHeatmap.ts:63,64,75,76`; `useAnalysisProjection.ts:75`; justified re-mints at `MergedDeltaPanel.vue:259,276,288` | ephemeral (per-render) | bounded ~340 | Sound by design. Conversion to `PlyIndex` through the single named boundary `colorMoveToPly` (`useTriangularHeatmap.ts:103`). Rationale at `src/types/game.ts:23-42`. |
| `PlyIndex` | number | derived index — 0-indexed position into a `variationPath: NodeId[]` (PlyIndex 0 = root) | output of `colorMoveToPly` (`useTriangularHeatmap.ts:103`); endpoints in `BoardState.analysisRange: [PlyIndex, PlyIndex]` (`src/types/game.ts:110`) | ephemeral; range endpoints persist | bounded ~340 | Sound by design; same pair as above. |
...
| Name | Prim. | Origin | Construction | Lifetime | Cardinality | Status / notes |
|------|-------|--------|-------------|----------|-------------|----------------|
| `ProfileId` | string | server UUID (intended) / **NIL-UUID sentinel (actual)** | **only** the NIL-UUID sentinel: `store/index.ts:81,691`, `defaults.ts:602` (`NIL_UUID as ProfileId`). **No re-brand from a real server value exists anywhere in `src/`.** | persisted (the identity field on `ProfileState`/`SessionState`) | 1 | **`[under-determined]`** — see prose below. |
| `SessionId` | string | server UUID (intended) / **NIL-UUID sentinel (actual)** | **only** the sentinel: `store/index.ts:80,690` | persisted | 1 | Same as `ProfileId`. |
