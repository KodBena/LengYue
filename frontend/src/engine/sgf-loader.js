...
export function loadSgf(sabakiOutput) {
    ...
    if (rootNode.delta?.setupOverwritten) {
        for (const posKey of Object.keys(rootNode.delta.setupOverwritten)) {
            ...
            if (rootNode.properties.AB?.includes(sgf))
                state.stones[posKey] = 'B';
            else if (rootNode.properties.AW?.includes(sgf))
                state.stones[posKey] = 'W';
        }
    }
    return state;
}
function transform(sabakiNode, parentId, nodes, size) {
    ...
    let move = null;
    if (props.B)
        move = sgfToMove(props.B[0], 'B', size);
    else if (props.W)
        move = sgfToMove(props.W[0], 'W', size);
    const node = {
        id,
        parent: parentId,
        children: [],
...
function hydrate(nodeId, nodes, stones, koPoint, size) {
    const node = nodes[nodeId];
    if (!node)
        return;
    let nextStones = { ...stones };
    let nextKo = null;
    const setupOverwritten = {};
    const processSetup = (coords, color) => {
        if (!coords)
            return;
        for (const sgfCoord of coords) {
            ...
            if (color)
                nextStones[key] = color;
            else
                delete nextStones[key];
        }
    };
    ...
    if (node.move && node.move.type === 'place') {
        ...
        if (result.ok) {
            ...
            console.warn(`[SgfLoader] illegal move skipped at node=${nodeId} ` +
