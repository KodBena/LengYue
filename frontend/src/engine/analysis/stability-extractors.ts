...
export const extractScoreLeadSign: StabilityExtractor<number> = (packet) => {
  const sl = packet.rootInfo?.scoreLead;
  if (sl === undefined || sl === null || Number.isNaN(sl)) return null;
  if (sl > 0) return 1;
  if (sl < 0) return -1;
  return 0;
};
...
export const extractWinrateQuintile: StabilityExtractor<number> = (packet) => {
  const wr = packet.rootInfo?.winrate;
  if (wr === undefined || wr === null || Number.isNaN(wr)) return null;
...
export const extractSearchAgreesWithPolicy: StabilityExtractor<boolean> = (packet) => {
  const mi = packet.moveInfos;
  if (!mi || mi.length === 0) return null;
  let topPriorMove: string | null = null;
  let topPrior = -Infinity;
  for (const m of mi) {
    const p = m.prior;
    if (p === undefined || p === null || Number.isNaN(p)) continue;
    if (p > topPrior) {
      topPrior = p;
      topPriorMove = String(m.move);
    }
  }
  if (topPriorMove === null) return null;
...
export const extractTop1Move: StabilityExtractor<string> = (packet) => {
  const mi = packet.moveInfos;
  if (!mi || mi.length === 0) return null;
  const mv = mi[0].move;
...
export const extractTop3Set: StabilityExtractor<string> = (packet) => {
  const mi = packet.moveInfos;
  if (!mi || mi.length < 3) return null;
  const moves: string[] = [];
  for (let i = 0; i < 3; i++) {
    const mv = mi[i].move;
...
export const extractTop2MarginQuintile: StabilityExtractor<number> = (packet) => {
  const mi = packet.moveInfos;
  if (!mi || mi.length < 2) return null;
  const visits: number[] = [];
