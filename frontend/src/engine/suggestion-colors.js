 ...
 */
const placeholderFn = (_t, alpha = 1.0) => `rgba(128, 128, 128, ${alpha})`;
/**
 ...
 */
export const getIntensityColor = shallowRef(placeholderFn);
/**
 ...
 */
export const getIntensityColorLinear = shallowRef(placeholderFn);
 ...
 */
function rotateHueLab(r, g, b, deg) {
    if (!deg)
        return [r, g, b];
    ...
    function fi(t) { return t > 0.206897 ? t * t * t : (t - 16 / 116) / 7.787; }
    let fy = f(y / Yn);
 ...
 */
function colorAtU(u, hueShiftDeg, alpha) {
 ...
 * when we replace it.
 */
function rebuildIntensityColorFn() {
    ...
    const linearFn = (intensity, alpha = 1.0) => {
    ...
    const ecdfFn = (intensity, _alpha = 1.0) => {
 ...
 */
export function setVisitDistribution(distributionData) {
 ...
 */
export function setIntensityHueShift(deg) {
    _hueShiftDeg = deg;
    rebuildIntensityColorFn();
}
// ─── Internal helpers ────────────────────────────────────────────────────────
 ...
 */
export const defaultVisitColor = (visitRatio) => {
