 ...
 */
export function scoreLeadToBlackPositive(scoreLead, overrideSettings, currentPlayer) {
...
export function clampKomi(value) {
    if (value < KOMI_MIN)
        return KOMI_MIN;
    if (value > KOMI_MAX)
        return KOMI_MAX;
 ...
 */
export function computeEvenKomi(input) {
    const scoreLeadBlackPositive = scoreLeadToBlackPositive(input.scoreLead, input.overrideSettings, input.currentPlayer);
