 ...
 */
export function resolveWinrateFraming(overrides) {
    if (!overrides)
        return 'SIDETOMOVE';
    const v = overrides.reportAnalysisWinratesAs;
    if (typeof v !== 'string')
        return 'SIDETOMOVE';
    if (WINRATE_FRAMINGS.includes(v)) { // widen the literal-union tuple to string[] for the .includes membership test
        return v; // membership confirmed above: v is one of the WINRATE_FRAMINGS literals
    }
    console.warn(`[winrate-framing] Unknown reportAnalysisWinratesAs value '${v}'; ` +
 ...
 */
export function normalizePacketToWhiteFraming(packet, framing) {
    let flip = false;
    if (framing === 'BLACK') {
        flip = true;
    }
...
const negateScalar = (v) => -v;
const oneMinus = (v) => 1 - v;
/**
 ...
 */
function flipMoveSignedFields(mi) {
    const flipped = {
        ...mi,
...
function flipRootSignedFields(ri) {
    const flipped = {
        ...ri,
...
function flipIfPresent(obj, key, fn) {
    const v = obj[key];
    if (typeof v === 'number') {
        obj[key] = fn(v);
    }
