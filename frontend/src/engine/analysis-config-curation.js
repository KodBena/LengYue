...
const CURATED_STDLIB_NAMES = new Set([
    // Reductions
 ...
 */
export function rewriteSymbolBody(body) {
    return body.replace(NP_CALL_PATTERN, (match, fnName) => CURATED_STDLIB_NAMES.has(fnName) ? `${fnName}(` : match);
}
/**
 ...
 */
export function rewriteSymbolBlock(symbols) {
    let rewriteCount = 0;
    const residue = [];
    const out = {};
 ...
 */
export function rewriteGradingParameterAnalysisConfig(gradingParameter) {
    const noop = {
        gradingParameter,
        rewriteCount: 0,
        residue: [],
    };
    if (!gradingParameter || typeof gradingParameter !== 'object')
        return noop;
    const gp = gradingParameter; // checked non-null object above; open-record for field reads
    const data = gp.data;
    if (!data || typeof data !== 'object')
        return noop;
    const dataObj = data; // checked non-null object above; open-record for field reads
    const config = dataObj.analysis_config;
    if (!config || typeof config !== 'object')
        return noop;
    const configObj = config; // checked non-null object above; open-record for field reads
    const symbols = configObj.symbols;
    if (!symbols || typeof symbols !== 'object')
        return noop;
    const result = rewriteSymbolBlock(symbols); // checked non-null object above; open-record for the symbol-block rewrite
