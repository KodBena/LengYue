...
<style scoped>
/* `min-height: 28px` + `flex-wrap: wrap` (iter-13, audit Finding G):
   at narrow viewports the metric cluster previously got crushed
   while `engine-identity` / `engine-controls` (both flex-shrink: 0)
   kept their full width — labels became unreadable, CONNECT clipped
   off the right edge. Wrapping lets the toolbar grow vertically
   instead. `justify-content: space-between` still spreads items
   within each wrapped row. */
