...
// ── Mint-time extension hook (komi calibration lives outside this file) ──
// The modal shell has no Go-specific content of its own; a component
// supplied via the `komiCalibration` named slot (wired up at the call
// site) registers itself here and `submit()` runs whatever was
// registered, without knowing what it does. See
// `KomiCalibrationControls.vue` and
// docs/notes/design/di-refactor-entanglement-investigation-2026-07-20.md §2.6.
let beforeSubmitHook: ((draft: CardCreatePayload) => Promise<void>) | null = null;
function registerBeforeSubmitHook(fn: ((draft: CardCreatePayload) => Promise<void>) | null) {
  beforeSubmitHook = fn;
}
...
async function submit() {
  ...
  try {
    // Run whatever mint-time extension registered itself via the
    // `komiCalibration` slot (see `registerBeforeSubmitHook` above). If
    // it throws (e.g. a komi-calibration evaluation failure), it has
    // already logged its own diagnostic message; we ABORT the mint
    // loudly (ADR-0002) via the catch below and the card is NOT created.
    if (beforeSubmitHook) {
      await beforeSubmitHook(draft.value);
    }
    ...
    <div class="modal-content">
      ...
      <div class="modal-body" v-if="draft">
        ...
        <div class="form-grid">
          ...
          <!-- Mint-time extension slot (e.g. Go-specific komi
               calibration). The modal shell has no knowledge of what
               fills this slot — the call site wires a component here
               that reads `boardId` and calls `registerBeforeSubmit` to
               hook into `submit()`. -->
          <slot name="komiCalibration" :board-id="draftBoardId" :register-before-submit="registerBeforeSubmitHook" />
        </div>
