 ...
 * `src/assets/setup-wizard-demo.json` (a synthetic self-play
 * position — the model playing itself, the analysed turn selected by
 * maximum visit-distribution entropy; no external game record, per
 * ledger row 735 — provenance header inside the asset itself). ZERO network, ZERO engine: the asset already
...
export interface SetupWizardDemoProvenance {
  /** Human-readable origin, e.g. "synthetic-self-play (…)". */
  readonly origin: string;
  readonly model: string;
  readonly selfPlayVisitsPerMove: number;
  readonly analysisVisits: number;
  readonly rules: string;
  readonly komi: number;
  /** How the analysed turn was chosen (entropy selection). */
...
function assertShape(raw: unknown): asserts raw is typeof demoAsset {
  ...
  // Field-level provenance checks (review finding: the display layer
  // interpolates these — a shape drift must fail HERE, loudly, not
  // render "undefined" strings; exactly what the Go-Seigen → self-play
  // asset swap witnessed). Checks mirror SetupWizardDemoProvenance.
  const pv = r.provenance as Record<string, unknown>;
  for (const k of ['origin', 'model', 'rules', 'selection', 'capturedAt'] as const) {
    if (typeof pv[k] !== 'string') fail(`"provenance.${k}" is missing or not a string`);
  }
  for (const k of ['komi', 'selfPlayVisitsPerMove', 'analysisVisits', 'selectedTurn', 'entropyBits'] as const) {
    if (typeof pv[k] !== 'number') fail(`"provenance.${k}" is missing or not a number`);
  }
}
