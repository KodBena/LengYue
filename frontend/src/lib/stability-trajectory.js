...
export const UNKNOWN = Symbol('stability-trajectory-unknown');
 ...
 */
export function appendObservation(t, V, value) {
    ...
    const tagged = value === null ? UNKNOWN : value;
 ...
 */
export function fromObservations(observations) {
    const t = emptyTrajectory();
 ...
 */
function* walkWindow(t, V_term, V_upper) {
    const cps = t.changepoints;
    let prev_V = V_term;
    let prev_val = valueAt(t, V_term);
 ...
 * metric result.
 */
function countChanges(t, V_term, V_upper) {
    let n = 0;
    let prev_val = valueAt(t, V_term);
    const cps = t.changepoints;
    let i = bisectRight(t, V_term) + 1;
    while (i < cps.length && cps[i].V <= V_upper) {
        ...
        if (prev_val !== UNKNOWN && cur !== UNKNOWN && prev_val !== cur)
            n++;
        prev_val = cur;
        i++;
    }
 ...
 */
export function anchoredAtVTerm(t, V_term, options = {}) {
 ...
 * end of the search.
 */
export function anchoredAtVMax(t, V_term, options = {}) {
    const V_upper = options.V_max ?? t.V_max;
    const threshold = options.threshold ?? 0.97;
    const nChanges = countChanges(t, V_term, V_upper);
    if (V_term <= 0 || V_upper <= V_term) {
 ...
 */
export function longestRunFraction(t, V_term, options = {}) {
    const V_upper = options.V_max ?? t.V_max;
    const threshold = options.threshold ?? 0.97;
    const nChanges = countChanges(t, V_term, V_upper);
    if (V_term <= 0 || V_upper <= V_term) {
    ...
    for (const seg of walkWindow(t, V_term, V_upper)) {
        ...
        if (weight > 0 && seg.value !== UNKNOWN) {
            totalKnown += weight;
            const key = seg.value; // narrowed: the `seg.value !== UNKNOWN` guard above excludes the Unknown sentinel
 ...
 */
export function changeRateInverse(t, V_term, options = {}) {
    const V_upper = options.V_max ?? t.V_max;
    const threshold = options.threshold ?? 0.97;
    const nChanges = countChanges(t, V_term, V_upper);
    if (V_term <= 0 || V_upper <= V_term) {
...
export const STABILITY_METRICS = new Map([
...
export const STABILITY_METRIC_LABELS = new Map([
...
export const DEFAULT_METRIC_ID = 'anchored_at_v_term';
export const STABILITY_METRIC_EXPLANATIONS = {
    anchored_at_v_term: "What fraction of the log-V window [V_term, V_max] carries the same extractor value as observed at V_term? The design note's canonical metric — captures \"does the engine's opinion at V_term survive further search?\" Sensitive to early flux when the engine hasn't yet committed.",
    anchored_at_v_max: "Same machinery but anchored at the value observed at V_max. Captures \"did the final answer emerge early?\" — high when the engine settled into the final value quickly; low when the final value only appeared near the end of the search.",
    longest_run_fraction: 'Of all values observed in [V_term, V_max], the log-V duration of the longest-held one as a fraction of the window. Anchor-independent: catches "the engine settled on something" regardless of which value or when. Robust to chaotic early flux that eventually concentrates.',
    change_rate_inverse: 'Volatility flavour: 1 / (1 + nChanges / log(V_max/V_term)). Anchor-independent. Value 1.0 = no transitions in the window; ~0.5 = about one transition per log-doubling of visits. Asymptotes toward 0 as the trajectory thrashes. Captures "how chaotic", not "how anchored".',
};
...
export function changeCount(t, V_lower = 0, V_upper = Infinity) {
    let count = 0;
    for (const cp of t.changepoints) {
        if (cp.V > V_lower && cp.V <= V_upper)
            count++;
    }
    return count;
}
/** V of the most recent changepoint, or null if empty. */
export function lastChangeV(t) {
    const cps = t.changepoints;
