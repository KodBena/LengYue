...
/**
 * The minimal sink surface the backstop drives. Injected so the de-dup
 * logic is exercisable against spies (the integration test) without a
 * real store, a real `i18n`, or a real `console`.
 */
export interface RejectionBackstopDeps {
  /** Push a user-visible system message. Real impl: store's `pushSystemMessage`. */
  pushSystemMessage: (type: 'error' | 'warning' | 'info', text: string) => void;
  /** Translate an i18n key + params. Real impl: `i18n.global.t`. */
  translate: (key: string, params?: Record<string, unknown>) => string;
  /** Developer-surface log sink. Real impl: `console.error`. */
  logError: (...args: unknown[]) => void;
  /**
   * Cap on the number of DISTINCT reasons that surface to the system log
   * within one latch lifetime. Bounds the worst case — an unbounded
   * stream of distinct rejection reasons — so even that cannot wipe the
   * 50-message log. Default 8: enough to show a handful of genuinely
   * different async failures, small relative to the log's 50-slot cap.
   */
  maxDistinctSurfaced?: number;
}

const DEFAULT_MAX_DISTINCT_SURFACED = 8;

/**
 * A handle over the latch. `handle` is the per-rejection entry point;
 * `reset` clears the de-dup state (the analog of the enrichment-merge
 * latch's `purgeAll` — see the de-dup note below).
 */
export interface RejectionBackstop {
 ...
 */
function dedupKey(reason: unknown): string {
 ...
 */
export function createRejectionBackstop(deps: RejectionBackstopDeps): RejectionBackstop {
  ...
  function handle(reason: unknown): void {
    ...
    if (surfacedKeys.has(key)) return;
