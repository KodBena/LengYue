...
function skipWhitespace(c) {
    while (c.pos < c.src.length && /\s/.test(c.src[c.pos]))
        c.pos++;
}
function peek(c) {
    return c.pos < c.src.length ? c.src[c.pos] : '';
}
function consume(c, ch) {
    if (peek(c) !== ch)
        fail(c, `Expected '${ch}' but found '${peek(c) || 'EOF'}'`);
    c.pos++;
}
function parseValue(c) {
    ...
    if (ch === '[')
        return parseArray(c);
    if (ch === '"' || ch === "'")
        return parseString(c, ch);
    if (ch === '-' || (ch >= '0' && ch <= '9'))
        return parseNumber(c);
    if (isIdentStart(ch))
        return parseIdentOrHole(c);
...
function parseObject(c) {
    ...
    for (;;) {
        ...
        const k = peek(c);
        if (k !== '"' && k !== "'")
            fail(c, `Expected string key, found '${k || 'EOF'}'`);
...
function parseIdentOrHole(c) {
    const start = c.pos;
    while (c.pos < c.src.length && isIdentPart(c.src[c.pos]))
        c.pos++;
...
function formatValue(v, indent) {
    ...
    return '{\n' + entries
        ...
        .map(([k, val]) => pad + JSON.stringify(k) + ': ' + formatValue(val, indent + 1))
        .join(',\n') + '\n' + close + '}';
}
export function validate(pipeline, declarations) {
    const errors = [];
    const warnings = [];
...
function walkHoles(v, path, visit) {
    if (isHole(v)) {
        visit(v, path || '$');
        return;
    }
...
export function substitute(pipeline, values) {
    ...
    return pipeline.map(stage => substituteValue(stage, values));
}
function substituteValue(v, values) {
    if (isHole(v)) {
        if (!(v.$param in values))
            throw new UnboundHoleError(v.$param);
        return values[v.$param];
    }
    if (Array.isArray(v))
        return v.map(x => substituteValue(x, values));
    if (v !== null && typeof v === 'object') {
        const out = {};
        // Each entry value is a `Value` by the AST's recursive shape.
        for (const [k, val] of Object.entries(v))
...
export function hasHoles(pipeline) {
    let found = false;
    // PipelineStageWithHoles[] widens to `Value` for the generic AST walker.
    walkHoles(pipeline, '', () => { found = true; });
    return found;
}
