 ...
 *
 * Scope as of 2026-08-06 (macro-public-id-tokens rolling-archive
 * move): migrations 1 → 2 through 63 → 64 (63 entries). The first
 * eight covered pre-v1.0.0 schema evolution; the rest are the
 * v1.0.x – v1.1.x active cycle, archived in per-PR rolling fashion
 * under the same archive contract.
 *
...
export const archivedMigrations: Migration[] = [
  ...
  // precedent above) rather than load-bearing — it keeps the persisted
  // shape honest instead of leaning on the read-site fallback. 'shared'
  // is the only sensible default: it is the view every pre-existing
  // workspace already had (the feature introduces two ADDITIONAL views,
  // not a replacement one), so this migration is a pure additive seed
  ...
  //
  // Archived from migrations.ts (rolling-archive move, macro-public-
  // id-tokens / schema-version 66) — see that file's active body for
  // the two current style anchors (64 → 65, 65 → 66).
  (blob: any) => {
    const out = structuredClone(blob);
