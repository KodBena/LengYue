 ...
 */
import { archivedMigrations } from './archived-migrations';
...
export const migrations = [
    ...
    (blob) => {
        ...
        const adaptive = witnessedContainer(out, 'profile.settings.engine.katago.adaptiveReevaluate');
        if (adaptive) {
            const a = adaptive;
...
export function migrate(blob) {
    ...
    if (version > CURRENT_SCHEMA_VERSION) {
        throw new Error(`Persisted blob is at schemaVersion ${version}, ahead of this app's ` +
    ...
    while (version < CURRENT_SCHEMA_VERSION) {
        ...
        if (!m) {
            throw new Error(`No migration registered for schemaVersion ${version} → ${version + 1}. ` +
