...
export const store = reactive({
    ...
    session: {
        ...
        id: '00000000-0000-0000-0000-000000000000', // NIL-UUID brand mint
        profileId: '00000000-0000-0000-0000-000000000000', // NIL-UUID brand mint
        ui: defaultSessionUI,
        reviews: {},
    },
    engine: {
...
export const activeBoardSize = computed(() => {
 ...
 */
export const boardsById = computed(() => {
...
export function mutateBoard(boardId, fn) {
 ...
 */
export function setSelectedModel(label) {
    store.engine.selectedModel = label;
}
export function mutateReviewSession(boardId, fn) {
 ...
 */
export function toggleCardTreeManualExpand(boardId, key) {
 ...
 */
export function setCardTreeManualExpand(boardId, keys) {
    if (keys.length === 0) {
...
export function addBoard(boardState) {
 ...
 */
const BOARD_SCOPED_STORE_CELLS = [
...
export function boardScopedStoreCellLabels() {
...
export function closeBoard(boardId) {
    ...
    analysisPersistenceService.discard(boardId).catch(() => { });
    ...
    // before the splice.
    for (const cell of BOARD_SCOPED_STORE_CELLS)
        cell.clear(boardId);
...
export function updateBoardState(index, newState) {
...
export function identityScopedCacheLabels() {
...
export function resetWorkspace() {
    ...
    // tenancy reset.
    for (const cache of IDENTITY_SCOPED_CACHES)
        cache.clear();
    ...
    store.session = {
        id: NIL_UUID, // NIL-UUID brand mint (logged-out sentinel)
        profileId: NIL_UUID, // NIL-UUID brand mint (logged-out sentinel)
...
export function updateFromRemote(remoteData) {
    ...
    delete migrated._pendingMigrationMessages;
    if (migrated.boards) {
    ...
    if (migrated.profile)
        store.profile = deepMerge(store.profile, migrated.profile);
    if (migrated.session)
        store.session = deepMerge(store.session, migrated.session);
...
export function pushSystemMessage(type, text) {
    const msg = {
    ...
    if (store.engine.messages.length > 50)
        store.engine.messages.pop();
}
export function clearSystemMessages() {
    store.engine.messages = [];
}
export function dismissSystemMessage(id) {
