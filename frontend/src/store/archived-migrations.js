 ...
 */
import { rewriteSymbolBlock, rewriteGradingParameterAnalysisConfig, } from '../engine/analysis-config-curation';
import { generateUUID } from '../lib/utils';
import { detectBrowserLocale, isSupportedLocale, } from '../i18n/locales';
...
export const archivedMigrations = [
    ...
    (blob) => {
        ...
        const NEW_SYMBOLS = {
            ...
            winrate_loss_topvsuser: '(x[0]["moveInfos"][0]["winrate"] - x[0]["userMoveInfo"]["winrate"]) if x[0]["userMoveInfo"] else 0',
            scoreLead_loss_topvsuser: '(x[0]["moveInfos"][0]["scoreLead"] - x[0]["userMoveInfo"]["scoreLead"]) if x[0]["userMoveInfo"] else 0',
            ...
            risk_adjusted_score_loss: 'safe((x[0]["moveInfos"][0]["scoreLead"] - (x[0]["userMoveInfo"]["scoreLead"] if x[0]["userMoveInfo"] else x[0]["moveInfos"][0]["scoreLead"])) / x[0]["rootInfo"]["scoreStdev"])',
        ...
        if (defaultPalette) {
            ...
            const matchesBrokenSeed = defaultPalette.delta_fn === 'quality_delta' &&
    ...
    (blob) => {
        ...
        const symbols = ae.symbols && typeof ae.symbols === 'object'
            ? ae.symbols
            : (ae.symbols = {});
        const V7_SCORELEAD_LOSS = '(x[0]["moveInfos"][0]["scoreLead"] - x[0]["userMoveInfo"]["scoreLead"]) if x[0]["userMoveInfo"] else 0';
        const NEW_SCORELEAD_LOSS = 'player_sign(x[0]) * ((x[0]["rootInfo"]["scoreLead"] - x[0]["userMoveInfo"]["scoreLead"]) if x[0]["userMoveInfo"] else 0)';
        const NEW_PLAYER_SIGN = '1.0 if x["rootInfo"]["currentPlayer"] == "B" else -1.0';
    ...
    (blob) => {
        ...
        if (out.session?.ui) {
            ...
            const validMode = (v) => v === 'instant' || v === 'sequential' || v === 'window';
            const validAnnotation = (v) => v === 'none' || v === 'from1' || v === 'fromCurrent';
    ...
    (blob) => {
        ...
        if (cardSets && typeof cardSets === 'object') {
            const cleanArray = (v) => Array.isArray(v) && v.every(n => typeof n === 'number') && v.length > 0
                ? v
                : null;
        ...
        if (out.session?.ui) {
            const validArr = (v) => Array.isArray(v) && v.every(n => typeof n === 'number');
    ...
    (blob) => {
        ...
        if (liveSymbols && typeof liveSymbols === 'object') {
            const result = rewriteSymbolBlock(liveSymbols);
        ...
        if (reviews && typeof reviews === 'object') {
            for (const [boardId, sessionData] of Object.entries(reviews)) {
                ...
                for (let cardIdx = 0; cardIdx < queue.length; cardIdx++) {
                    ...
                    if (!card || typeof card !== 'object')
                        continue;
                    const result = rewriteGradingParameterAnalysisConfig(card.gradingParameter);
                    if (result.rewriteCount > 0) {
                        card.gradingParameter = result.gradingParameter;
                        totalRewrites += result.rewriteCount;
                    }
                    for (const r of result.residue) {
                        residueLocations.push(`session.reviews.${boardId}.queue[${cardIdx}].symbols.${r.name}`);
                    }
                }
            }
        }
        ...
        if (totalRewrites > 0) {
            messages.push({
                type: 'info',
                text: `v1.0.3 curation alignment: rewrote ${totalRewrites} symbol body` +
        ...
        if (residueLocations.length > 0) {
            ...
            messages.push({
                type: 'warning',
                text: `v1.0.3 curation alignment: ${residueLocations.length} symbol ` +
    ...
    (blob) => {
        ...
        if (ui && typeof ui === 'object') {
            ...
            const valid = nav && typeof nav === 'object' &&
    ...
    (blob) => {
        ...
        if (katago && typeof katago === 'object') {
            ...
            const isPlainObject = existing !== null &&
    ...
    (blob) => {
        ...
        if (katago && typeof katago === 'object') {
            ...
            if (!isPlainObject) {
                ...
                if (typeof existing.worstQuantile !== 'number')
                    existing.worstQuantile = 0.05;
                if (typeof existing.extraVisits !== 'number')
                    existing.extraVisits = 800;
            }
        }
        return out;
    },
    ...
    (blob) => {
        ...
        const reviews = out.session?.reviews;
        if (reviews && typeof reviews === 'object') {
            for (const sessionData of Object.values(reviews)) {
                if (!sessionData || typeof sessionData !== 'object')
                    continue;
                const queue = sessionData.queue;
                if (!Array.isArray(queue))
    ...
    (blob) => {
        ...
        if (settings && typeof settings === 'object') {
            ...
            if (appearance && typeof appearance === 'object') {
                if (typeof appearance.ownershipOpacityCeiling !== 'number') {
                    appearance.ownershipOpacityCeiling = 0.55;
                }
            }
            // 1b. Watchdog animation duration (ms).
            const katago = settings.engine?.katago;
            if (katago && typeof katago === 'object') {
                if (typeof katago.watchdogAnimationMs !== 'number') {
                    katago.watchdogAnimationMs = 500;
                }
            }
    ...
    (blob) => {
        const out = structuredClone(blob);
        const analysisEnv = out.profile?.settings?.engine?.katago?.analysis_env;
        ...
        if (analysisEnv &&
            ...
            typeof knobs === 'object' &&
            !Array.isArray(knobs)) {
            const parameterMeta = analysisEnv.parameter_meta;
            if (parameterMeta && typeof parameterMeta === 'object' && !Array.isArray(parameterMeta)) {
                ...
                for (const name of Object.keys(meta)) {
                    ...
                    const qeuboControlled = entry.qeubo_controlled === true;
                    target[knobId] = {
                        id: knobId,
                        label: name,
                        domain: 'qeubo',
    ...
    (blob) => {
        ...
        if (knobs && typeof knobs === 'object' && !Array.isArray(knobs)) {
            ...
            for (const knobId of Object.keys(target)) {
                if (!knobId.startsWith('qeubo.'))
                    continue;
                const decl = target[knobId];
                if (!decl || typeof decl !== 'object')
                    continue;
                if (decl.domain === 'qeubo') {
                    decl.domain = 'palette';
                }
            }
        }
    ...
    (blob) => {
        ...
        if (knobs && typeof knobs === 'object' && !Array.isArray(knobs)) {
            ...
            for (const [knobId, defaultPriority] of Object.entries(priorities)) {
                ...
                if (!decl || typeof decl !== 'object')
                    continue;
                const current = decl.priority;
                if (typeof current === 'number' && Number.isFinite(current))
                    continue;
                decl.priority = defaultPriority;
            }
        }
        return out;
    },
    ...
    (blob) => {
        ...
        const knobs = out.profile?.settings?.knobs;
        if (knobs && typeof knobs === 'object' && !Array.isArray(knobs)) {
            const decl = knobs['engine.first-report-during-search-after'];
            if (decl && typeof decl === 'object') {
                const inputs = decl.inputs;
                if (Array.isArray(inputs) && inputs.length > 0) {
                    const first = inputs[0];
                    if (first && typeof first === 'object') {
                        const f = first.minFloor;
                        if (typeof f !== 'number' || !Number.isFinite(f)) {
                            first.minFloor = 0.035;
                        }
                    }
                }
            }
    ...
    (blob) => {
        ...
        if (ui && typeof ui === 'object') {
            ...
            const isPlainObject = cur !== null && cur !== undefined &&
    ...
    (blob) => {
        ...
        if (katago && typeof katago === 'object') {
            const adaptive = katago.adaptiveReevaluate;
        ...
        if (env && Array.isArray(env.palettes)) {
            for (const p of env.palettes) {
                if (!p || typeof p !== 'object')
                    continue;
                const existing = p.delta_ordering;
                if (existing === 'lower_is_worse' || existing === 'higher_is_worse')
                    continue;
    ...
    (blob) => {
        ...
        if (nav) {
            ...
            const alreadyPerBoard = typeof sel === 'object' && sel !== null && !('kind' in sel);
            if (!alreadyPerBoard) {
                nav.selection = {};
            }
        }
