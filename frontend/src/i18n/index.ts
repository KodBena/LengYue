 ...
 * Total catalog payload at full sweep is on the order of 200 KB
 * across all five locales (per the 150-300 string inventory in
 * docs/notes/i18n-plan.md, ~50 bytes per entry × 5 locales).
 * Lazy loading adds a Promise round-trip on every locale switch
 * and a Suspense-or-loader concern in every consumer. The payload
 * fits the budget; bundling keeps the consumer surface synchronous
 * (`$t('key')` resolves immediately, no awaits, no loading
 * states). If catalogs balloon past ~500 KB total, revisit.
 *
 * ── Cold-start locale ────────────────────────────────────────────
