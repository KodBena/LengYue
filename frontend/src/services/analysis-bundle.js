 ...
 */
export function parseStorageError(err) {
    ...
    if (status === 413 && kind === 'bundle_too_large') {
        if (typeof body.request_bytes !== 'number' || typeof body.cap_bytes !== 'number')
            return null;
        return {
    ...
    if (status === 413 && kind === 'user_quota_exceeded') {
        if (typeof body.current_bytes !== 'number' || typeof body.quota_bytes !== 'number')
            return null;
        return {
 ...
 * Pure function: no I/O, no side effects, no shared state.
 */
export function asStorageError(err) {
 ...
 */
function recogniseStorageUnion(err) {
    ...
    if (e.kind === 'bundle_too_large' && e.status === 413) {
        if (typeof e.requestBytes !== 'number' || typeof e.capBytes !== 'number')
            return null;
        return err; // validated bundle_too_large member
    }
    if (e.kind === 'user_quota_exceeded' && e.status === 413) {
        if (typeof e.currentBytes !== 'number' || typeof e.quotaBytes !== 'number')
            return null;
        return err; // validated user_quota_exceeded member
    }
    if (e.kind === 'unknown_scheme' && e.status === 500) {
        if (typeof e.scheme !== 'string')
            return null;
        return err; // validated unknown_scheme member
    }
    return null;
}
...
const ENRICHMENT_PLACEHOLDER_RAW = {
    ...
    moveInfos: [],
    rootInfo: { winrate: 0, scoreLead: 0, visits: 0, currentPlayer: 'B' },
};
/**
 ...
 */
export function projectLedgerToBundle(boardId) {
    ...
    const rawRecords = ledger.listRawForNodes(nodeIds).map(r => ({
        configHash: `${RAW_PREFIX}${r.rawKey}`,
        nodeId: r.nodeId,
        packet: r.raw,
    }));
    const enrichmentRecords = ledger.listEnrichmentForNodes(nodeIds).map(e => ({
 ...
 * ADR-0002 forbids).
 */
export function replayBundleIntoLedger(bundle) {
    if (bundle.schemaVersion !== BUNDLE_SCHEMA_VERSION) {
        throw new Error(`analysis-bundle: unsupported schemaVersion ${bundle.schemaVersion}; ` +
    ...
    for (const r of bundle.records) {
        if (r.configHash.startsWith(RAW_PREFIX)) {
            ...
            const rawKey = r.configHash.slice(RAW_PREFIX.length);
            ...
            const enrichedKey = r.configHash.slice(ENR_PREFIX.length);
            if (r.packet.extra)
                ledger.recordEnrichment(enrichedKey, r.nodeId, r.packet.extra);
        }
        else {
            ...
            const enrichedKey = r.configHash;
            if (r.packet.extra)
                ledger.recordEnrichment(enrichedKey, r.nodeId, r.packet.extra);
            legacyRawDropped++;
        }
    ...
    if (legacyRawDropped > 0) {
        console.warn(`[analysis-bundle] legacy bundle: restored enrichment for ${legacyRawDropped} ` +
