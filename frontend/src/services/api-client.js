...
export class ApiClient {
    ...
    async request(method, path, body, options) {
        ...
        if (text === '')
            return undefined; // empty 204 body: callers of these routes declare request<void>, so undefined IS T (see comment above)
    ...
    async login(username, password) {
        ...
        const res = await this.request('POST', '/auth/token', params);
