...
import { resolveWinrateFraming, normalizePacketToWhiteFraming, } from '../engine/katago/winrate-framing';
import { parseVersionResponse, parseModelsResponse, requiresDeltaAnalysisRefusal, } from '../engine/katago/version-probe';
import { buildPerQueryCapabilities, shouldWarnTranspositionUnmet, } from '../engine/katago/capability-injection';
...
import { markEngineConnected, applyEngineDisconnectReset, setEngineInfo, refreshEngineVersion, recordPacketRate, markWatchdogPingPending, recordWatchdogPong, recordLastResponseBoard, } from './engine-connection';
...
import { compileAnalysisConfig, compileEngineOverrides, deriveAnalysisKeys, activeAnalysisKeys, } from '../state/analysis-config';
import { KATAGO_WS_URL } from '../config/env';
import { KATAGO_FIRST_REPORT_FLOOR_S, ENGINE_METRICS_TICK_MS, ENGINE_HEARTBEAT_POLL_MS, } from '../lib/timing';
...
export class AnalysisService {
    ...
    async probeEngineInfo() {
        try {
            ...
            if (requiresDeltaAnalysisRefusal(versionResult.capabilities)) {
                pushSystemMessage('error', i18n.global.t('analysis.proxyMissingDeltaAnalysis'));
                this.disconnect();
                return;
            }
            ...
            for (const degradation of versionResult.degraded) {
                console.warn('[AnalysisService] Capability advertisement failed typed-mirror validation; capability degraded:', degradation, versionResult.raw);
                pushSystemMessage('warning', i18n.global.t('analysis.capabilityDegraded', {
            ...
            const isSelectorMode = versionResult.capabilities !== null &&
            ...
            const useTransposition = !!store.profile.settings.engine
                .katago?.useTransposition;
            if (shouldWarnTranspositionUnmet(versionResult.capabilities, useTransposition)) {
                pushSystemMessage('warning', i18n.global.t('analysis.proxyMissingTransposition'));
            }
        }
        catch (err) {
        ...
        this.watchdogTimer = window.setInterval(async () => {
            ...
            if ('version' in resp && typeof resp.version === 'string'
                ...
                const versionPayload = (resp && typeof resp === 'object')
                    ? resp // checked object; widen the typed response to an open record (see comment above)
                    : null;
    ...
    async clearCache() {
        ...
        try {
            ...
            pushSystemMessage(lookupCacheOn ? 'warning' : 'info', i18n.global.t(lookupCacheOn ? 'engine.clearCache.okButLookupCache' : 'engine.clearCache.ok'));
        }
        catch (err) {
            pushSystemMessage('error', i18n.global.t('engine.clearCache.failed', { error: String(err) }));
        }
    }
    analyzeFullGame(boardId, visits) {
        const board = store.boards.find(b => b.id === boardId);
        if (!board || store.engine.status !== 'connected')
            return null;
    ...
    fullPath, startTurn, endTurn, visits, configOverride, overrideSettingsOverride, forReview = false, isRealtime = true) {
        const board = store.boards.find(b => b.id === boardId);
        if (!board || store.engine.status !== 'connected')
            return null;
        if (fullPath.length === 0 || endTurn < startTurn)
            return null;
        ...
        const keys = hasConfigOverride
            ? deriveAnalysisKeys(analysis_config, overrideSettings, store.engine.selectedModel ?? undefined)
        ...
        const hasOverrides = overrideSettings !== undefined && Object.keys(overrideSettings).length > 0;
        ...
        const query = {
            ...
            ...(isRealtime ? {
                ...
                firstReportDuringSearchAfter: Math.min(store.profile.settings.engine.katago.reportDuringSearchEvery, Math.max(KATAGO_FIRST_REPORT_FLOOR_S, store.profile.settings.engine.katago.firstReportDuringSearchAfter)),
            } : {}),
            analyzeTurns,
        ...
        this.restartCallbacks.set(queryId, () => {
    ...
    analyzeActiveNode(boardId, mode, visits, configOverride, overrideSettingsOverride) {
        const board = store.boards.find(b => b.id === boardId);
        if (!board || store.engine.status !== 'connected')
            return null;
        ...
        const ponderCeiling = mode === 'ponder'
     ...
     */
    routeSubscriptionResponse(res, queryId) {
        if ('error' in res) {
            console.warn(`[analysis-service] error packet routed onto analysis query ${queryId}; `
                + `surfaced via the global onError channel, releasing query bookkeeping:`, res.error);
            this.stopQuery(queryId);
            return;
        }
    ...
    onAnalysisUpdate(response, queryId) {
        this.packetCount++;
        if (DEBUG_PACKETS) {
            const q = this.activeQueries.get(queryId);
            const dt = q ? (performance.now() - q.startedAt).toFixed(1) : 'n/a';
            console.log(`[analysis-service] packet +${dt}ms queryId=${queryId} `
        ...
        if (nodeId) {
            ...
            if (extra)
                ledger.recordEnrichment(queryInfo.enrichedKey, nodeId, extra);
            ...
            if (import.meta.env.DEV)
                performance.measure('rb3:handler', { start: rb3Start });
        }
        ...
        // query).
        if (queryInfo.ponderCeiling !== undefined
            && !response.isDuringSearch) {
            pushSystemMessage('warning', i18n.global.t('analysis.ponderExhausted', {
     ...
     * on first insert.
     */
    indexQueryOnBoard(boardId, queryId) {
