...
function fromWireGameListItem(wire) {
    return {
        id: wire.id, // ACL Band-2 brand mint (wire id → GameSourceId)
...
function toWireImportItem(input) {
    return {
        raw_content: input.rawContent,
        source_path: input.sourcePath,
    };
}
function fromWireImportOutcome(wire) {
    switch (wire.status) {
        case 'created':
            return {
                status: 'created',
                gameId: wire.game_id, // ACL Band-2 brand mint
        ...
        default: {
            ...
            throw new Error(`Unknown ImportOutcome variant: ${JSON.stringify(_exhaustive)}`);
        }
    }
}
...
export class LibraryService {
    ...
    async listGames(query) {
        ...
        if (f.playerLike !== null)
            params.set('player_like', f.playerLike);
        if (f.playerWhiteLike !== null)
            params.set('player_white_like', f.playerWhiteLike);
        if (f.playerBlackLike !== null)
            params.set('player_black_like', f.playerBlackLike);
        if (f.dateFrom !== null)
            params.set('date_from', f.dateFrom);
        if (f.dateTo !== null)
            params.set('date_to', f.dateTo);
        if (f.resultEq !== null)
            params.set('result_eq', f.resultEq);
        if (f.rulesetEq !== null)
            params.set('ruleset_eq', f.rulesetEq);
        if (f.boardSizeEq !== null)
            params.set('board_size_eq', String(f.boardSizeEq));
        params.set('offset', String(query.offset));
        params.set('limit', String(query.limit));
        const wire = await api.request('GET', `/library/games?${params.toString()}`);
        return {
    ...
    async getGame(gameId) {
        try {
            const wire = await api.request('GET', `/library/games/${gameId}`, undefined, { silentStatuses: GET_GAME_SILENT });
    ...
    async deleteGame(gameId) {
        await api.request('DELETE', `/library/games/${gameId}`);
    }
    /**
     ...
     */
    async listPlayers() {
        const wire = await api.request('GET', '/library/players');
     ...
     */
    async importGames(inputs, onProgress) {
        if (inputs.length === 0)
            return [];
        const chunks = [];
        ...
        for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
            ...
            const wire = await api.request('POST', '/library/games/import', body);
