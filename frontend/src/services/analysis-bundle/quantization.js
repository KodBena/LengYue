 ...
 */
export function quantiseOwnershipQ4(ownership) {
    if (ownership.length !== OWNERSHIP_CELL_COUNT) {
        throw new Error(`quantiseOwnershipQ4: expected ${OWNERSHIP_CELL_COUNT}-cell ownership, ` +
    ...
    for (let i = 0; i < OWNERSHIP_CELL_COUNT; i++) {
        ...
        const byteIdx = i >>> 1;
        if ((i & 1) === 0)
            packed[byteIdx] |= idx & 0x0f;
        else
            packed[byteIdx] |= (idx & 0x0f) << 4;
    }
    return packed;
}
/**
 ...
 */
export function dequantiseOwnershipQ4(packed) {
    const out = new Array(OWNERSHIP_CELL_COUNT);
    for (let i = 0; i < OWNERSHIP_CELL_COUNT; i++) {
        const byteIdx = i >>> 1;
 ...
 */
export function quantiseOwnershipQ8(ownership) {
    if (ownership.length !== OWNERSHIP_CELL_COUNT) {
        throw new Error(`quantiseOwnershipQ8: expected ${OWNERSHIP_CELL_COUNT}-cell ownership, ` +
            `got ${ownership.length}`);
    }
 ...
 */
export function dequantiseOwnershipQ8(packed) {
    if (packed.length !== OWNERSHIP_CELL_COUNT) {
        throw new Error(`dequantiseOwnershipQ8: expected ${OWNERSHIP_CELL_COUNT}-byte packed, ` +
            `got ${packed.length}`);
    }
    const out = new Array(OWNERSHIP_CELL_COUNT);
    for (let i = 0; i < OWNERSHIP_CELL_COUNT; i++) {
 ...
 */
export function quantisePolicyQ8Factored(policy) {
    if (policy.length !== POLICY_CELL_COUNT) {
        throw new Error(`quantisePolicyQ8Factored: expected ${POLICY_CELL_COUNT}-cell policy, ` +
            `got ${policy.length}`);
    }
 ...
 */
export function dequantisePolicyQ8Factored(packed) {
    const { bitmap, legalCount, values } = packed;
    if (values.length !== legalCount) {
        throw new Error(`dequantisePolicyQ8Factored: legalCount=${legalCount} mismatches ` +
            `values.length=${values.length}`);
    }
    const out = new Array(POLICY_CELL_COUNT);
    let vi = 0;
    for (let i = 0; i < POLICY_CELL_COUNT; i++) {
    ...
    if (vi !== legalCount) {
        throw new Error(`dequantisePolicyQ8Factored: walked ${vi} legal bits but ` +
