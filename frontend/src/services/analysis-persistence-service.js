...
import { asBoardId, parseStorageError, projectLedgerToBundle, replayBundleIntoLedger, } from './analysis-bundle';
import { base64ToUint8Array, getEncoderForScheme, listKnownSchemes, uint8ArrayToBase64, } from './analysis-bundle/encoder';
...
const GET_SILENT = [404];
const PUT_SILENT = [413];
 ...
 */
function toWireBundleV2(bundle, scheme) {
    const encoder = getEncoderForScheme(scheme);
    if (!encoder) {
        throw new Error(`analysis-persistence-service: cannot encode for scheme '${scheme}'; ` +
    ...
    return {
        ...
        format_descriptor: encoded.descriptor,
...
function fromWireRecord(wire) {
    return {
        ...
        nodeId: wire.node_id, // ACL Band-2 brand mint (wire node_id → NodeId)
 ...
 * toWireBundleV2.
 */
function readCompressionScheme() {
    ...
    if (scheme === 'v2-projected')
        return 'json-projected-v1';
    if (scheme === 'v2-quantized')
        return 'ownership-q4-policy-q8-factored-v1';
    if (scheme === 'v2-quantized-hifi')
        return 'ownership-q8-policy-q8-factored-v1';
...
function rethrowAsStorageError(err) {
...
export class AnalysisPersistenceService {
    ...
    dirtyVersions = reactive(new Map());
    ...
    autoSaveErrors = reactive(new Map());
    /**
    ...
    async save(boardId) {
        ...
        scheme === 'v1'
            ? toWireBundle(bundle) // widen v1 wire shape to the union
            : toWireBundleV2(bundle, scheme); // widen v2 wire shape to the union
        try {
            const wire = await api.request('PUT', `/analysis-bundles/${encodeURIComponent(boardId)}`, body, { silentStatuses: PUT_SILENT });
    ...
    async restore(boardId) {
        let wire;
        try {
            wire = await api.request('GET', `/analysis-bundles/${encodeURIComponent(boardId)}`, undefined, { silentStatuses: GET_SILENT });
        }
        catch (err) {
     ...
     */
    async discard(boardId) {
        await api.request('DELETE', `/analysis-bundles/${encodeURIComponent(boardId)}`);
    ...
    async refreshSummaries() {
        const wire = await api.request('GET', '/analysis-bundles');
