...
import { store, boardsVersion, updateFromRemote, pushSystemMessage, resetWorkspace, buildPersistencePayload, } from '../store';
...
export class SyncService {
     ...
     */
    connect() {
        watch(() => this.auth.state.value, (next) => this.onAuthStateChange(next), { immediate: true });
        this.startWatcher();
    }
    /**
    ...
    async hydrate(userId) {
        ...
        try {
            const doc = await api.request('GET', `/documents/${this.docKey}`);
    ...
    scheduleSync() {
        ...
        if (state.kind !== 'authenticated' || state.userId === undefined)
            return;
        if (this.hydratedForUserId !== state.userId)
            return;
        if (this.pendingTimer !== null)
            clearTimeout(this.pendingTimer);
