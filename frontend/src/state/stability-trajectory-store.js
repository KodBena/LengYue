 ...
 */
import { ref } from 'vue';
import { STABILITY_EXTRACTORS, } from '../engine/analysis/stability-extractors';
import { appendObservation, emptyTrajectory, } from '../lib/stability-trajectory';
import { store } from '../store';
const trajectories = new Map();
const trajectoryVersions = new Map();
function keyOf(rawKey, extractorId, nodeId) {
    return `${rawKey}|${extractorId}|${nodeId}`;
}
function getOrCreateVersion(key) {
...
function getOrCreateTrajectory(key) {
...
export class StabilityTrajectoryStore {
    ...
    record(rawKey, nodeId, packet) {
        ...
        for (const [extractorId, extract] of STABILITY_EXTRACTORS.entries()) {
            const value = extract(packet); // pin the map's generic <Q> extractor to a concrete callable for this packet
     ...
     */
    getTrajectory(rawKey, extractorId, nodeId) {
        const key = keyOf(rawKey, extractorId, nodeId);
    ...
    purgeBoard(boardId) {
        ...
        for (const key of Array.from(trajectories.keys())) {
            ...
            const nodeId = key.substring(key.lastIndexOf('|') + 1);
