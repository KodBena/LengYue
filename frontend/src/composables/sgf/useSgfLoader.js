...
export function useSgfLoader() {
    async function loadFile(file) {
        try {
            ...
            if (store.session.ui.loadSgfAtLastNode) {
                ...
                if (path.length > 0)
                    navigateTo(newBoard, path[path.length - 1]);
            }
            addBoard(newBoard);
        }
        catch (err) {
    ...
    function openFileDialog() {
        ...
        input.accept = '.sgf';
        input.onchange = (e) => {
            const file = e.target.files?.[0]; // DOM: onchange set on the file <input> just created, so target is it
            if (file)
                void loadFile(file);
        };
        input.click();
