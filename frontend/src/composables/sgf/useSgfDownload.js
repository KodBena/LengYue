/**
 * src/composables/sgf/useSgfDownload.ts
...
// ── Composable ────────────────────────────────────────────────────────────────
export function useSgfDownload() {
    function downloadActiveBoard() {
 ...
 */
function deriveFilename(board) {
    const root = board.nodes[board.rootNodeId];
    const props = root?.properties ?? {};
    const sanitise = (s) => s.trim().replace(/[^a-zA-Z0-9_-]+/g, '_').replace(/^_+|_+$/g, '');
    const pb = props['PB']?.[0];
    const pw = props['PW']?.[0];
    const dt = props['DT']?.[0];
