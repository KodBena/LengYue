...
/** The accessor the preview leaf (`ChartPreviewBox` / `FloatingThumbnail`)
 *  invokes inside its own render scope, so the cache subscription stays
 *  leaf-local (ADR-0010 read-locality). Null while no node is targeted or
 *  on a cache miss. */
export type SnapshotAccessor = () => BoardSnapshot | null;

export interface PreviewSnapshot {
  /** The visible gate: the *target node*, written only synchronously. */
  readonly previewNode: Ref<NodeId | null>;
  /** Derives the displayed snapshot through the synchronous cache read. */
  readonly getPreview: SnapshotAccessor;
  /** Point the preview at `nodeId`: set the gate synchronously, then warm the
   *  shared cache fire-and-forget (cache-only write; never the gate). */
  readonly showPreview: (nodeId: NodeId) => void;
  /** Clear the gate (a leave / no-target). Synchronous; no warm needed. */
  readonly reset: () => void;
}
 ...
 */
export function usePreviewSnapshot(boardId: BoardId): PreviewSnapshot {
  ...
  const getPreview: SnapshotAccessor = () =>
 ...
 */
export function warmSnapshotAccessor(nodeId: NodeId, boardId: BoardId): SnapshotAccessor {
  const { getSnapshot, getSnapshotSync } = useThumbnailCache();
