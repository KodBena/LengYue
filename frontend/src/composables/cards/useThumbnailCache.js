...
import { cacheSnapshot, getCachedSnapshot, isPathWarm, markPathWarm, } from './thumbnail-render-resources';
// ── Private pure helper ────────────────────────────────────────────────────
...
async function generateSnapshot(nodeId, boardId) {
    ...
    const tempState = {
        ...
        nodes: Object.fromEntries(Object.entries(rootBoard.nodes).map(([id, node]) => [id, { ...node }])),
    };
...
export function useThumbnailCache() {
    ...
    async function warmPath(nodeIds, boardId) {
        if (isPathWarm(nodeIds))
            return;
        markPathWarm(nodeIds);
        await Promise.all(nodeIds.map(id => getSnapshot(id, boardId)));
    }
    /**
     ...
     */
    function variationMarkerLabels(nodeId, boardId) {
        const rootBoard = store.boards.find(b => b.id === boardId);
