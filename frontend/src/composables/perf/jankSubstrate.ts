 ...
 */
export async function loadShusakuGame(): Promise<LibraryGame> {
  const baseQuery = {
    sort: 'date' as const,
  ...
  if (!row) {
    const summary = attempts
      ...
      .join('\n');
    throw new Error(
      '[jank-substrate] Could not find the fixed Shusaku game ' +
  ...
  if (!game) {
    throw new Error(
      `[jank-substrate] Shusaku row #${row.id} matched the list query but GET ` +
 ...
 */
export async function sampleRandomGames(
  count: number,
...
export function loadGameAsNewBoard(rawContent: string, clientGameId: BoardId | null): BoardId {
  storeCreateBoard();
...
export function forwardToMove(boardId: BoardId, target: number): void {
 ...
 */
export async function setUpRail(): Promise<BoardId> {
  ...
  if (others.length < TOTAL_BOARDS - 1) {
    console.warn(
      `[jank-substrate] library returned only ${others.length} other games; ` +
  ...
  if (plies !== SHUSAKU.expectedMoves) {
    console.warn(
      `[jank-substrate] matched Shusaku game has ${plies} moves, ` +
 ...
 */
export function startHoverScrub(): { stop: () => void } {
  let running = true;
