 ...
 */
function assertAllOverlaysOn() {
    const s = readOverlayState();
    const off = Object.entries(s)
        .filter(([, on]) => !on)
        .map(([k]) => k);
    if (off.length > 0) {
        throw new Error(`[jank-extended] overlay assertion failed: ${off.join(', ')} not enabled. ` +
...
function restoreOverlayState(s) {
    const ui = store.session.ui;
...
function inflightStillRunning(queryId) {
    ...
    return {
        ...
        async run(ctx) {
            ...
            try {
                ...
                await ctx.measure('drive', () => runAutonav({ markPrefix: 'jankext:autonav', normalizeTab: true }).done);
                scrub.stop();
                ...
                if (!stillInFlight) {
                    console.warn('[jank-extended] in-flight 100000-visit query finished before the autonav ' +
