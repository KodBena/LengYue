...
function createScenarioContext(name) {
    ...
    let savedAdaptiveEnabled;
    function mark(markName, detail) {
        performance.mark(`scenario:${name}:${markName}`, detail !== undefined ? { detail } : undefined);
    }
    function requireActiveBoardId(where) {
        const b = activeBoard.value;
        if (!b)
            throw new Error(`scenarioContext.${where}: no active board after create`);
        return b.id;
    }
    function makeHandle(queryId) {
        ...
        const settled = waitForCondition(() => !telemetry.inFlight.value.some((q) => q.queryId === queryId));
        return {
            queryId,
            settled,
    ...
    const ctx = {
        ...
        async loadLibraryGameById(gameId) {
        ...
        async connectEngine(opts = {}) {
            ...
            if (opts.model) {
                ...
                if (matches.length !== 1) {
                    throw new Error(`scenarioContext.connectEngine: model "${opts.model}" matched ${matches.length} of [${labels.join(', ')}]; need exactly one`);
                }
        ...
        analyzeRange(boardId, opts) {
            const board = store.boards.find((b) => b.id === boardId);
            if (!board)
                throw new Error(`scenarioContext.analyzeRange: board ${boardId} not found`);
