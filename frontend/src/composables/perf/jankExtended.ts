 ...
 *
 * BASELINE PORT (commit 34650471, pre-refactoring tree). The protocol is
 * byte-for-byte the same as the main-side `jankExtended.ts`; the ONLY
 * divergence is the profile-write mechanism: this tree predates
 * `src/store/profile-owner.ts`, so the transposition-capability write is a
 * DIRECT `store.profile.settings.engine.katago.useTransposition = …`
 * assignment rather than the owner-routed `mutateProfile(…)` the main tree
 * uses. The two writes are behaviourally identical (same deep-reactive object,
 * same SyncService observation) — this is the same delta the baseline
 * `scenarioContext.ts` already carries for the adaptive toggle. Named here and
 * in the worklog as the (sole) port delta so it is not mistaken for a protocol
 * difference.
 *
 ...
 * transposition CAPABILITY is a DIRECT profile write in this baseline tree
 * (no `profile-owner.ts` / `mutateProfile` here — see the port-delta note in
 * the file header).
 */
function enableAllOverlays(): void {
  ...
  // Transposition CAPABILITY (wire `transposition: {}` opt-in). Default true,
  // but set explicitly per the assert-not-assume posture; the capability only
  // engages when the proxy advertises it. DIRECT write (baseline port delta).
  store.profile.settings.engine.katago.useTransposition = true;
}
...
function restoreOverlayState(s: OverlayState): void {
  ...
  store.profile.settings.engine.katago.useTransposition = s.useTransposition;
}
