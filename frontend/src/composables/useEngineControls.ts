...
export interface EngineControls {
  /** True when the engine WebSocket is open and ready. */
  readonly isConnected: ComputedRef<boolean>;
  /** The raw status string, for components that need finer-grained display. */
  readonly status: ComputedRef<EngineStatus>;
  /** Live engine metrics: packets/sec, latency, watchdog timestamp. */
  readonly metrics: ComputedRef<EngineMetrics>;
  /** Open the WebSocket and start the watchdog. */
  connect: () => void;
  /** Terminate the WebSocket and clear all active analyses. */
  disconnect: () => void;
