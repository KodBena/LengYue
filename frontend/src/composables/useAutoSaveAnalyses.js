...
export function useAutoSaveAnalyses() {
    ...
    // session.
    const lastScheduledVersion = new Map();
    ...
    const boardWatcherStops = new Map();
    ...
    async function fireSave(boardId, scheduledAtVersion) {
        ...
        // paused.
        if (analysisPersistenceService.autoSaveErrorFor(boardId))
            return;
    ...
    function scheduleSaveIfNeeded(boardId) {
        if (analysisPersistenceService.autoSaveErrorFor(boardId))
            return; // paused
    ...
    function setupBoardWatcher(boardId) {
        ...
        // unconditionally.
        if (boardWatcherStops.has(boardId))
            return;
        const stop = watch(
    ...
    function teardownBoardWatcher(boardId) {
    ...
    const stopReconcile = watch(boardsSetVersion, () => {
        ...
        for (const id of currentIds) {
            if (!boardWatcherStops.has(id))
                setupBoardWatcher(id);
        }
    }, { immediate: true });
    ...
    const stopGateWatch = watch(() => isGated(), (gated) => {
