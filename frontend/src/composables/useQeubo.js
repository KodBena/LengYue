...
// ─── Module-scoped state ─────────────────────────────────────────────────────
const _statusRef = ref(null);
const _pairRef = ref(null);
const _bestRef = ref(null);
// null = unknown (pre-bootstrap or transient error); true = backend
// has QEUBO_ENABLED=true; false = backend returned 503.
const _calibrationEnabledRef = ref(null);
const _isBusyRef = ref(false);
/**
 ...
 */
function paramNameForKnobId(knobId) {
 ...
 */
function ensureKnobDecl(name, range) {
...
function reconcileQeuboKnobs() {
    ...
    mutateProfile((profile) => {
        ...
        for (const [name, meta] of Object.entries(pm)) {
            ...
            const hasValidRange = Array.isArray(range) &&
                range.length === 2 &&
        ...
        for (const knobId of Object.keys(knobs)) {
            if (!knobId.startsWith(KNOB_PREFIX))
                continue;
 ...
 */
function acquireExperimentClaims(controlledParams) {
    const acquired = [];
    ...
    for (const name of controlledParams) {
        ...
        if (!range) {
            ...
            for (const a of acquired)
                releaseKnob(a, QEUBO_CONSUMER_ID);
            throw new Error(`PBO: parameter "${name}" has no range — cannot synthesize a KnobDecl.`);
        }
        ...
        if (result.kind === 'rejected') {
            ...
            for (const a of acquired)
                releaseKnob(a, QEUBO_CONSUMER_ID);
            throw new Error(`PBO: cannot claim knob "${knobId}" — currently held by ` +
 ...
 * satisfies.
 */
function releaseAllExperimentClaims() {
    for (const knobId of _claimedKnobIds) {
        ...
        releaseKnob(knobId, QEUBO_CONSUMER_ID);
    }
 ...
 */
function rehydrateExperimentClaims() {
    ...
    if (_statusRef.value === null)
        return;
    const pm = store.profile.settings.engine.katago.analysis_env.parameter_meta ?? {};
    for (const [name, meta] of Object.entries(pm)) {
        if (meta?.qeubo_controlled !== true)
            continue;
        if (!meta?.range)
            continue;
        const knobId = ensureKnobDecl(name, meta.range);
        if (_claimedKnobIds.has(knobId))
            continue;
        const result = claimKnob(knobId, {
            consumerId: QEUBO_CONSUMER_ID,
            policy: 'hard',
        ...
        if (result.kind === 'rejected') {
            console.warn(`[useQeubo] rehydrate: cannot re-claim knob "${knobId}" — ` +
...
const _toolbarView = computed({
    ...
    get: () => (store.session.ui.qeuboToolbarView ?? 'applied'),
    set: (v) => {
        store.session.ui.qeuboToolbarView = v;
    },
});
// ─── Computeds ───────────────────────────────────────────────────────────────
const _experimentExists = computed(() => _statusRef.value !== null);
const _phase = computed(() => _statusRef.value?.phase ?? 'idle');
const _initProgress = computed(() => {
    const s = _statusRef.value;
    if (!s || s.phase !== 'init')
        return null;
    return { done: s.initIndex, total: s.numInitQueries };
});
const _optimizationProgress = computed(() => {
    const s = _statusRef.value;
    if (!s || s.phase !== 'optimization')
        return null;
 ...
 */
const _effectiveParameterValues = computed(() => {
    ...
    if (view === 'A' && pair)
        return { ...base, ...pair.valuesA };
    if (view === 'B' && pair)
        return { ...base, ...pair.valuesB };
    return { ...base };
});
/**
 ...
 */
const _appliedParameterValues = computed(() => ({ ...store.profile.settings.engine.katago.analysis_env.parameters }));
// ─── Lifecycle ───────────────────────────────────────────────────────────────
/**
...
async function startNewExperiment(controlledParams) {
    ...
    for (const name of controlledParams) {
        const meta = pm[name];
        if (!meta?.range) {
            throw new Error(`PBO: parameter "${name}" has no [min, max] range in parameter_meta. ` +
...
async function submitPreference(preferred) {
    const pair = _pairRef.value;
    if (!pair) {
...
function pinCurrent(name) {
    ...
    const bookmark = {
        id: generateUUID(), // BookmarkId brand mint (fresh client UUID)
        name: trimmed,
        createdAt: Date.now(),
        parameters,
    };
...
function applyBookmark(id) {
    ...
    for (const knobId of Object.keys(bookmark.parameters)) {
        if (!(knobId in registry))
            continue;
    ...
    if (conflicts.length > 0) {
        pushSystemMessage('warning', i18n.global.t('qeuboInternal.bookmarkApplyRefused', {
    ...
    mutateProfile((profile) => {
        ...
        for (const [knobId, value] of Object.entries(bookmark.parameters)) {
            if (knobId in registry) {
        ...
        Object.keys(bookmark.parameters).map(paramNameForKnobId));
