...
const KNOB_ALLOWED_PATH_PREFIXES = ['profile.', 'session.ui.'];
export function useAppBootstrap(auth) {
    ...
    watch(() => store.profile.settings.appearance.theme, (theme) => document.documentElement.setAttribute('data-theme', theme), { immediate: true });
    ...
    watch(() => store.profile.settings.engine.katago.analysis_env.parameter_meta, () => {
        reconcileQeuboKnobs();
    ...
    watch(() => store.profile.settings.appearance.locale, (loc) => {
    ...
    watch(() => auth.state.value, (next, prev) => {
    ...
    watch(() => qeubo.toolbarView.value, () => analysisService.restartActiveAnalyses());
    ...
    watch(() => auth.state.value, (next, prev) => {
        ...
        const isAuth = next.kind === 'authenticated';
        if (isAuth && !wasAuth) {
            analysisPersistenceService.refreshSummaries().catch(() => { });
        }
        else if (!isAuth && wasAuth) {
    ...
    // (secondary causes).
    const restoredBoards = new Set();
    const restoreWatcherStops = new Map();
    function setupRestoreWatcher(boardId) {
        if (restoreWatcherStops.has(boardId))
            return;
        const stop = watch(
        ...
        () => analysisPersistenceService.summaryFor(boardId), (summary) => {
            if (auth.state.value.kind !== 'authenticated')
                return;
            if (restoredBoards.has(boardId))
                return;
            if (!summary)
                return;
            restoredBoards.add(boardId);
            analysisPersistenceService.restore(boardId).catch(() => { });
        }, { immediate: true });
        restoreWatcherStops.set(boardId, stop);
    }
    function teardownRestoreWatcher(boardId) {
    ...
    watch(boardsSetVersion, () => {
        const currentIds = new Set();
        for (const board of store.boards)
            currentIds.add(board.id);
        // Snapshot keys before iterating because teardown mutates
        // the map.
        for (const boardId of [...restoreWatcherStops.keys()]) {
            if (!currentIds.has(boardId))
                teardownRestoreWatcher(boardId);
        }
        for (const id of currentIds) {
            if (!restoreWatcherStops.has(id))
                setupRestoreWatcher(id);
        }
    }, { immediate: true });
    onMounted(async () => {
