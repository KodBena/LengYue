 ...
 * card as IDLE.
 */
export function fixedNetworkPolicy(opts) {
    return async (board, _card) => {
 ...
 * whole run.
 */
export async function runAutonomousDriver(opts) {
    const errorBudget = opts.errorBudget ?? 5;
    const boardIdRef = ref(opts.boardId);
    ...
    await waitForCondition(() => session.state.value === 'AWAITING_MOVE' || session.state.value === 'IDLE');
    ...
    for (let cardIndex = 0; cardIndex < opts.cards.length; cardIndex++) {
        ...
        while (session.state.value === 'AWAITING_MOVE' && !(opts.shouldStop?.())) {
            ...
            if (recordedScore !== null)
                perCardScores.push(recordedScore);
            ...
            const userColor = postNode?.move?.color ?? 'B';
        ...
        const finalState = session.state.value;
