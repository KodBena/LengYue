...
import { findGameByHead } from './useEngineResponder';
import { isReviewTransientState } from '../review/useReviewSession';
export function useBoardMoveRouting(reviewSession, engineResponder) {
    function handleBoardMove(x, y) {
     ...
     */
    function handlePastePv(pv) {
        if (reviewSession.state.value === 'AWAITING_MOVE')
            return;
        if (isReviewTransientState(reviewSession.state.value))
            return;
        if (!activeBoard.value || pv.length === 0)
            return;
        let board = activeBoard.value;
