...
import { store, mutateBoard, updateBoardState, pushSystemMessage, } from '../../store';
 ...
 */
export function findGameByHead(board, headNodeId) {
    for (const [startNodeId, session] of Object.entries(board.games)) {
        if (session.currentHeadNodeId === headNodeId) {
            return { startNodeId: startNodeId, session }; // re-brand: board.games is keyed by NodeId; Object.entries widens the key to string
        }
    }
    return null;
}
/** Engine's color = whichever the user is not playing. */
export function engineColorFor(userColor) {
    return userColor === 'B' ? 'W' : 'B';
}
export function useEngineResponder() {
    const { t } = useI18n();
    const inFlight = new Set();
    async function fireAndAdvanceHead(boardId, gameStartNodeId) {
        if (inFlight.has(boardId))
            return;
        ...
        try {
            ...
            if (beforeMove.currentNodeId !== board.currentNodeId)
                return;
            // Re-check session — user may have ended it mid-query.
            if (!beforeMove.games[gameStartNodeId])
                return;
            ...
            mutateBoard(boardId, draft => {
                ...
                if (s)
                    s.currentHeadNodeId = next.currentNodeId;
            });
        }
        catch (err) {
