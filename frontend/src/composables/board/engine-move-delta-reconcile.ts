...
import type { BoardId, GameNode, NodeId } from '../../types';
import { store, mutateBoard } from '../../store';
import { navigateTo } from '../../engine/navigator';

/**
 * Per-move delta shape consumed by `reconcileEngineMoveDelta`. Both
 * `EngineMoveApplied` and `MatchMoveApplied` have this shape; the
 * helper accepts either via structural compatibility.
 */
 ...
 *                 "usePlayFromPosition").
 */
export function reconcileEngineMoveDelta(
  boardId: BoardId,
  delta: EngineDelta,
  label: string,
): void {
  ...
  mutateBoard(boardId, (draft) => {
    ...
    if (parent === undefined) {
      throw new Error(
        `${label}: parent node ${previousPointer} missing from board ${boardId}`,
      );
    }
    if (newNode !== null) {
