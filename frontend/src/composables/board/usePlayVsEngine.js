...
import { engineColorFor } from './useEngineResponder';
export function usePlayVsEngine(engineResponder) {
    ...
    const activeBoardGameHeadIds = computed(() => {
        const board = activeBoard.value;
        if (!board)
            return undefined;
