...
import { BEST_MOVE_COLOR, getIntensityColor, CLUSTER_PALETTES } from '../../engine/suggestion-colors';
...
function formatVisitsCompact(visits) {
    return _visitsCompactFormatter.format(visits);
}
export function gtpToBoard(gtp) {
 ...
 */
export function useMoveSuggestions(getNodeId) {
    const packet = computed(() => ledger.getRaw(activeAnalysisKeys.value.rawKey, getNodeId()));
    ...
    const suggestions = computed(() => {
        ...
        if (!p || !p.moveInfos || p.moveInfos.length === 0)
            return [];
        ...
        const visibleMoves = p.moveInfos.map((info, moveIndex) => ({ info, moveIndex })).filter(({ info }) => {
            try {
                return filterFn(info, p.rootInfo, uiContext);
            }
            catch {
                return false;
            }
    ...
    function buildPvMoves(moveIndex) {
        const p = packet.value;
        if (!p || !p.moveInfos || !p.rootInfo)
            return [];
        ...
        for (let i = 0; i < info.pv.length; i++) {
            ...
            if (!coords)
                continue;
            const color = i % 2 === 0 ? firstPlayer : (firstPlayer === 'B' ? 'W' : 'B');
