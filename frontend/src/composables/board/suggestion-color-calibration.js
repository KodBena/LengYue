...
import { setVisitDistribution, setIntensityHueShift, } from '../../engine/suggestion-colors';
 ...
 */
async function loadVisitDistribution() {
    try {
        const data = await getResource('visit-distribution');
 ...
 */
export function initSuggestionColorCalibration() {
    ...
    watch(() => store.profile.settings.appearance.intensityHueShift, (deg) => setIntensityHueShift(deg), { immediate: true });
