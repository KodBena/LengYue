...
import { EnrichedAccumulator, EMPTY_ENRICHED, } from './enriched-accumulator';
/** The active palette's state_fn names — the series-seed set. */
function activeSeedNames() {
...
export function useEnrichedData(pathIdsRef) {
    const acc = new EnrichedAccumulator();
    const out = shallowRef(EMPTY_ENRICHED);
    ...
    watch([pathIdsRef, () => activeAnalysisKeys.value.enrichedKey, () => JSON.stringify(activeSeedNames())], rebuild, { immediate: true });
    ...
    const stopFlush = onLedgerFlush((changedKeys) => {
        ...
        for (const key of changedKeys) {
            ...
            if (keyPart !== rawKey && keyPart !== enrichedKey)
                continue;
            const nodeId = key.slice(sep + 1); // brand: the ledger key's `:`-suffix is the NodeId it was built from
            if (acc.patchNode(nodeId, ledger.getCombined(rawKey, enrichedKey, nodeId)))
                dirty = true;
        }
        if (dirty)
            out.value = acc.snapshot();
