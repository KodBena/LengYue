/**
 * src/composables/analysis/useAnalysisProjection.ts
 ...
 */
export function useAnalysisProjection(boardId) {
    // 1. Source Data
    const variationPath = useVariationPath(() => boardId);
    const enriched = useEnrichedData(variationPath);
    const { visitVector, selectionRange, setSelectionRange, analyzeSelection, } = useAnalysisTimeline(variationPath, boardId);
    ...
    const getPlayerIndex = (color) => {
        const board = store.boards.find(b => b.id === boardId);
        if (!board || activeMainIndex.value === null)
            return null;
        ...
        if (currentNode?.move?.color === color)
            return null;
        let count = 0;
