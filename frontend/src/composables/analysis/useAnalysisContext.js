...
export function useAnalysisContext(boardId) {
    ...
    const mistakeGapHistogramSeries = computed(() => [
...
export const AnalysisContextKey = Symbol('AnalysisContext');
/** Create the context for `boardId` and provide it to the subtree. */
export function provideAnalysisContext(boardId) {
 ...
 */
export function injectAnalysisContext() {
    const ctx = inject(AnalysisContextKey);
    if (!ctx) {
        throw new Error('injectAnalysisContext: no AnalysisContext in scope — analysis panels must render within an AnalysisDashboard (provideAnalysisContext) subtree.');
    }
    return ctx;
}
