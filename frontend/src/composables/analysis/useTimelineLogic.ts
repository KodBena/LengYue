...
export interface Segment {
  ...
  values: number[]; // Raw values within this segment
  stats: {
    min: number;
    max: number;
    mean: number;
...
export function useTimelineLogic(dataVector: Ref<number[]>, debounceMs = TIMELINE_SELECTION_DEBOUNCE_MS) {
  ...
  const segments = computed(() => {
    ...
    const createSegment = (s: number, e: number) => {
   ...
   */
  const selectionRange = ref<[number, number]>([0, Math.min(dataVector.value.length, 100)]);
  
  /**
   * Debounced selection range for "driving" heavy components like ECharts.
   */
  const debouncedRange = ref<[number, number]>([...selectionRange.value]);

  const updateDebounced = debounce((val: [number, number]) => {
