 ...
 */
function matchesTarget(packet, turnNumber) {
    return (packet && packet.isDuringSearch === false && packet.turnNumber === turnNumber)
        ? packet
        : null;
}
export function waitForAnalysis(rawKey, nodeId, turnNumber, options) {
    return new Promise((resolve, reject) => {
        ...
        const onAbort = () => settle(() => reject(new AnalysisWaitError('aborted')));
        const unwatch = watch(() => ledger.getRaw(rawKey, nodeId), (packet) => {
