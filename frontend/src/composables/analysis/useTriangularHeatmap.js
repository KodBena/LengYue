...
export function useTriangularHeatmap(variationPath) {
    return computed(() => {
        const matrix = [];
        let min = Infinity;
        let max = -Infinity;
        variationPath.value.forEach((nodeId) => {
            ...
            enr.black?.triangular?.forEach(([[s, t], v]) => {
                const cell = {
                    color: 'B',
                    s: s, // brand mint: triangular tuple index → ColorMoveIndex
                    t: t, // brand mint: triangular tuple index → ColorMoveIndex
                    value: v,
                };
...
export const colorMoveToPly = (m, color) => 
