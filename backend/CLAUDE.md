...
The local form for the backend: this file, the umbrella `CLAUDE.md`,
`tests/CLAUDE.md` when test-authoring is in scope, every cited ADR,
`docs/handoff-current.md`, `docs/notes/tenancy.md` when Band 2/3 work
is in scope, and any open dispatch under `docs/dispatch/` addressed to
the backend are read end to end before substantive work — not skimmed
for keywords, not relied on through search-result fragments or IDE
previews. If reading is deferred for a budget reason, say so audibly —
...
Migrations are Alembic revisions under `alembic/versions/`, applied
automatically by the lifespan in `main.py` on every backend start
via `db.alembic_bootstrap.bootstrap_alembic`. The bootstrap probes
the live DB's state against `REVISION_MARKERS`, stamps
`alembic_version` at the right revision for installs not yet
Alembic-managed, then runs `alembic upgrade head`.

For new schema work:

1. Edit `db/schema.py` (the SQLAlchemy `Table` declarations).
2. `alembic revision --autogenerate -m "describe change"` — drafts
   a revision file under `alembic/versions/`.
3. **Review the autogenerated file**. Alembic's autogenerate misses
   renames (detects them as drop+add — data-destroying), custom
   column types, server-default changes that don't round-trip, and
   some constraint nuances. Hand-edit the draft where needed.
4. Test: `alembic upgrade head && alembic downgrade -1 && alembic
   upgrade head`. Confirm the schema returns to the expected state.
5. If the revision adds a column whose presence should make the
   bootstrap probe stamp at this revision (or later), append a
   `(table, column, revision_id)` entry to `REVISION_MARKERS` in
   `db/alembic_bootstrap.py`. Without this, end-users upgrading
   across multiple revisions in one restart get stamped at an
   earlier revision than they actually are.
6. Commit the schema change, the revision file, and any
   `REVISION_MARKERS` update together.

The legacy `scripts/migrate_*.py` files predate Alembic adoption
and stay as historical record — they're the path pre-v1.0 installs
take to reach the baseline the bootstrap can stamp from. New
schema changes do not add to that directory.
