...
## Discharge 2 — baseline regeneration (coordinator directive,
ledger row 2498 arc)

The coordinator's disposition: the ORIGINAL `layout-audit-baseline.json`
was itself the defective artifact — captured while a live backend on
this host answered the SPA's default `http://localhost:8764`, silently
laundering "cold boot" into "cold boot, plus whatever a reachable
backend supplies." It never ratcheted the SPA's real no-backend state at
all; discharge 1's own 172-finding wave (§ above) is that real state,
finally visible for the first time. Directive: regenerate the baseline
under the isolated environment at all seven geometries, replace the
contaminated one, give it real provenance, verify the ratchet still
catches a deliberate new violation, re-run all gates, and enumerate the
new baseline's own per-rule/per-geometry counts so the 172 findings feed
an L5/cure pipeline rather than vanishing into a bulk baseline write.

### `--emit-baseline` — a genuinely reproducible generation command

`scripts/layout-audit.mjs` gains `--emit-baseline`: after the normal
seven-geometry audit run, writes `layout-audit-baseline.json` directly
from that run's own collected keys (`emitBaseline`), with a
`_provenance` block carrying:

- `generatedByCommand`: `node scripts/layout-audit.mjs --build --emit-baseline`
  — literally the command run to produce this file, for the first time
  honestly (see below).
- `environment.contract`: the deterministic cold-boot contract
  (`VITE_API_BASE_URL` baked to a probed-dead port `>= 19000` at build
  time, via this same dispatch's own `pickDeadBackendPort`).
- `geometries`: the seven geometry labels.
- `generatedAt` / `branchSha` (`git rev-parse HEAD`, read directly by
  the script itself via `execFileSync`, never hand-typed).
- `regenerationNote`: one sentence recording why THIS regeneration
  happened (the contaminated predecessor, ledger row 2498 arc), per the
  coordinator's own instruction — worded so a FUTURE regeneration
  replaces this note with its own reason rather than accumulating a
  changelog inside the field.

**Discharges the gates review's own rider.** `lyt-adr0019-gates-review.md`
§9 named a real gap: the predecessor baseline's `_comment` claimed
"Generated by `node scripts/layout-audit.mjs --build --port 19300`," but
no flag existed at that time to make the script actually WRITE the
baseline file — it was hand-assembled from a report's own `keys`. The
new `_comment` and `_provenance.generatedByCommand` together name a
command that this dispatch verified, by running it, actually produces
the committed file byte-for-byte (re-run twice, identical `keys` array
both times — the `generatedAt`/timestamp fields are the only ones that
legitimately differ run to run).

### Regeneration, performed

```
node scripts/layout-audit.mjs --build --emit-baseline
```

Run from `frontend/`, branch tip at generation time
`a796dde1cc0ae35c69087c5e21049ab4fec91661` (this dispatch's own prior
commit — the baseline file is added in the NEXT commit, so it records
the SHA it was generated against, not a SHA that includes itself, which
would be circular). Wrote 223 keys, matching discharge 1's own isolated
figure exactly.

### Green-by-construction, verified

`npm run layout-audit` (the `--build --check` CI path) against the new
baseline: **exit `0`**, `223 total findings, 0 new vs baseline` at
every one of the seven geometries. WITNESSED directly (not inferred from
the emission step alone — a separate, full re-run).

### Ratchet-still-red verification (one quick authored probe, reverted)

Per the coordinator's own instruction — regenerating a baseline that
absorbs 172 new keys must not have accidentally fail-opened the gate.
Temporarily added one CSS rule to `src/App.vue`'s existing (unscoped)
`<style>` block:

```css
/* TEMP PROBE -- ratchet verification only, reverted before commit */
#lyt-presence-menu-btn { width: 4px !important; height: 4px !important; }
```

Ran `npm run layout-audit`: **exit `1`**, `226 total findings, 7 new vs
baseline` — one new `target-size::#lyt-presence-menu-btn` finding at
EACH of the seven geometries, named individually in the script's own
`NEW:` output. Reverted the probe immediately (`git diff --stat` on
`App.vue` confirmed empty afterward — the file is byte-identical to
before the probe). Re-ran `npm run layout-audit` a final time: **exit
`0`**, back to `223 total, 0 new`. The ratchet is genuinely live, not
merely present.

### Per-rule / per-geometry counts of the new baseline (223 keys) —
enumerated for the L5/cure pipeline, per the coordinator's own
instruction not to let them vanish into a bulk write

The per-geometry TOTAL column is the script's own `renderSummary`
console output verbatim (`npm run layout-audit`'s own text). The
per-rule columns are a deduped-key breakdown computed directly from
`layout-audit-report.json` — the same structured data the console
summary and `emitBaseline` both derive from, recombined with one small
Python pass (`set()` per geometry+rule, matching `emitBaseline`'s own
dedup semantics exactly), not a hand recount:

| geometry | total (deduped keys) | focus-invisible | pointer-occlusion | target-size | unreachable-control | viewport-escape |
|---|---|---|---|---|---|---|
| 2560×1440 | 52 | 10 | 25 | 15 | 2 | 0 |
| 1920×1080 | 33 | 4 | 17 | 10 | 2 | 0 |
| 1366×768 | 34 | 4 | 17 | 10 | 2 | 1 |
| 1024×768 | 24 | 1 | 13 | 7 | 2 | 1 |
| 900×600 | 24 | 1 | 13 | 7 | 2 | 1 |
| 480×900 | 23 | 1 | 13 | 7 | 2 | 0 |
| 1080×1920 | 33 | 4 | 17 | 10 | 2 | 0 |
| **total (deduped keys, all geometries)** | **223** | **25** | **115** | **66** | **14** | **3** |

**By rule, script's own native count** (raw finding entries per rule
across all seven geometries — this axis can exceed the deduped-key count
for a rule because the script's own stable-selector keying deliberately
collapses several DOM elements sharing an identical unidentified
tag+class ancestor chain onto ONE key within a geometry+rule pair, per
`scripts/layout-audit.mjs`'s own documented "accepted weakness" —
several colliding elements still each push one raw finding, which is
why `pointer-occlusion`'s raw 153 is a materially different number from
its deduped-key contribution to the 223 total):

```
target-size          101
pointer-occlusion     153
focus-invisible        28
unreachable-control    49
viewport-escape         3
                    ----- (raw finding total: 334; deduped-key total: 223)
```

**What these 223 keys ARE, disclosed precisely for the L5/cure
pipeline.** 12 keys are the pre-L4 baseline's own pre-existing residue
(the 1366×768 font-rendering drift already disclosed in L2b/L3/L4's own
prior reports). The remaining 211 keys are newly-baselined, previously
NEVER-audited findings — the bulk of them (the `pointer-occlusion`
cascade, 115 deduped keys) trace to ONE root cause per discharge 1's own
analysis: a first-run wizard modal (`div.wizard-card`) and an
authentication-error indicator (`button.auth-error.user-badge`) that
render, for the first time in this project's own audit history, because
cold boot is now genuinely backend-less. This is not 211 independent
defects — it is a small number of root UI states (the wizard modal's own
markup, the auth-error badge's own sizing/focus/contrast) whose
consequences fan out across every interactive element the modal
backdrop happens to sit in front of, at every geometry. A follow-up
dispatch scoped to "cold-boot auth/wizard surface" (named as the
recommended next item in discharge 1, above) would likely close a large
fraction of these 211 keys by fixing 2-3 root components, not by
triaging 211 rows individually — flagged here so the L5/cure pipeline
starts from that framing rather than treating the count as 211 unrelated
tickets.

### Gates, re-run at this discharge's own final tip
...
- **Full suite** (`NODE_OPTIONS=--max-old-space-size=2048 npx vitest run
  --maxWorkers=2`): exit `0`, **271 files passed | 3 skipped (274)**,
  **3368 passed | 8 skipped (3376)** — unchanged; this discharge touches
  no test file. WITNESSED.
- **`npm run layout-audit`**: exit `0`, **223 total findings, 0 new vs
  baseline**, at all seven geometries. **Green by construction.**
  WITNESSED, and the ratchet's own liveness independently re-verified
  (see the probe above) rather than trusted on the strength of the
  regeneration alone.

### Files touched, this discharge

- `frontend/scripts/layout-audit.mjs` (`--emit-baseline` flag,
  `emitBaseline`, `currentGitShaOrNull`, doc-header addendum)
- `frontend/layout-audit-baseline.json` (regenerated in full — 223 keys,
  real provenance header, replacing the contaminated 89-key predecessor)
- `src/App.vue` — touched transiently for the ratchet-liveness probe,
  reverted; zero net diff (confirmed via `git diff --stat`).
