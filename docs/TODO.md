...
#### `[frontend]` Cards tab merge — moved to Completed

Closed 2026-05-06 across two PRs as the design note suggested.
PR 1 shipped per-board forest state, composable signature
changes, and the orange overlay (schema migration 15 → 16
field rename); PR 2 shipped the UI restructure (schema migration
16 → 17 activeTab rewrite, new `ReviewSessionPanel`,
`ForestDirectory` host integration, App.vue tab collapse). The
plan's "if the user prefers a single PR, that is also fine"
escape hatch was not taken — the seam paid off for review
ergonomics. See the Frontend Completed table above for the
synopsis. Worklogs at
`docs/worklog/2026-05-06-cards-tab-merge-pr1-per-board-forest.md`
and `docs/worklog/2026-05-06-cards-tab-merge-pr2-tab-restructure.md`.
